/* =========================================================================
   i18n.js — Català · Castellà · English · Français · Deutsch · 中文 · العربية
   Idioma por defecto: català.
   ========================================================================= */
(function (global) {
  'use strict';

  const DICT = {

    /* ================= CATALÀ ================= */
    ca: {
      'html.lang': 'ca',
      'html.dir': 'ltr',
      'locale': 'ca-ES',
      'doc.title': 'Eclipsi AR 2026 · 12 d\'agost',

      'app.kicker': 'Eclipsi Solar Total',
      'app.title': '12 d\'agost de 2026',
      'app.subtitle': 'El primer eclipsi total visible des de la península ibèrica en més d\'un segle',
      'app.geo': 'Fer servir la meva ubicació GPS',
      'app.geoActive': 'Ubicació GPS activa',
      'app.geoLocating': 'Localitzant…',
      'app.geoError': 'No s\'ha pogut obtenir la ubicació: ',
      'app.geoHttps': 'Recorda que el GPS requereix HTTPS.',
      'app.geoUnsupported': 'El teu navegador no admet geolocalització.',
      'app.myLocation': 'La meva ubicació',
      'app.mapPoint': 'Punt del mapa',

      'card.phases': 'Fases de l\'eclipsi a la teva posició',
      'card.sim': 'Simulació en temps real',
      'card.sun': 'On és el Sol ara mateix',
      'card.local': 'Circumstàncies locals',
      'card.band': 'Franja de totalitat',
      'card.cities': 'Ciutats de referència',
      'card.safety': 'Seguretat i consells',
      'card.tools': 'Eines',

      'ro.mag': 'Magnitud',
      'ro.obs': 'Sol cobert',
      'ro.phase': 'Fase',
      'ro.total': 'TOTAL',
      'ro.partial': 'Parcial',
      'ro.before': 'Encara no',
      'ro.after': 'Acabat',

      'scrub.explore': 'Explorar l\'eclipsi',
      'scrub.live': 'EN DIRECTE',
      'scrub.drag': 'Arrossega per avançar / retrocedir',
      'scrub.back': 'Tornar al directe',

      'sun.az': 'Azimut (des del Nord)',
      'sun.alt': 'Altura sobre l\'horitzó',
      'sun.dir': 'Orientació',
      'sun.set': 'Posta de sol avui',
      'sun.below': '(sota l\'horitzó)',
      'sun.note': 'La brúixola mostra el <b>Nord a dalt</b>; la vora és l\'horitzó i el centre, el zenit. La línia taronja és el recorregut que farà el Sol durant l\'eclipsi, i el punt blanc marca l\'instant del màxim.',

      'legend.center': 'Línia central',
      'legend.band': 'Banda de totalitat',
      'legend.you': 'La teva posició',
      'map.note': 'La línia central més propera és a <b>{km} km</b> cap al <b>{dir}</b> ({lat}°, {lon}°). Toca el mapa per calcular qualsevol altre punt.',
      'map.noteSimple': 'Toca el mapa per calcular les circumstàncies a qualsevol punt.',
      'map.offline': 'Mapa no disponible sense connexió.',

      'cities.note': 'En <b>taronja</b>, les que són dins de la franja de totalitat.',

      'phases.note': 'Hores en la teva zona horària local. «Alt» és l\'altura del Sol sobre l\'horitzó en aquell instant.',
      'phases.noTotality': 'Sense fase de totalitat en aquesta ubicació: l\'eclipsi serà parcial i <b>mai</b> no et podràs treure el filtre.',

      'phase.c1': 'C1 · Primer contacte',
      'phase.c1d': 'La Lluna toca la vora del Sol. Comença l\'eclipsi parcial. <b>Filtre posat.</b>',
      'phase.c2': 'C2 · Inici de la totalitat',
      'phase.c2d': 'Anell de diamant i grans de Baily. <b>Ja et pots treure el filtre.</b>',
      'phase.max': 'Màxim de l\'eclipsi',
      'phase.maxd': 'Moment àlgid. Corona solar, planetes i estrelles visibles.',
      'phase.c3': 'C3 · Fi de la totalitat',
      'phase.c3d': 'Reapareix el Sol. <b>Filtre posat JA</b>, abans de la primera espurna.',
      'phase.c4': 'C4 · Últim contacte',
      'phase.c4d': 'La Lluna abandona el disc solar. Fi de l\'eclipsi.',

      'cd.c1': 'Manquen per a l\'INICI (C1)',
      'cd.c2': '⚡ Manquen per a la TOTALITAT (C2)',
      'cd.max': 'Manquen per al MÀXIM',
      'cd.c3': '🔴 Queda de TOTALITAT — mira ara!',
      'cd.c4': 'Manquen per al FINAL (C4)',
      'cd.done': '✨ L\'eclipsi ha acabat',
      'cd.notVisible': 'Eclipsi no visible des d\'aquí',
      'cd.days': 'Dies', 'cd.hours': 'Hores', 'cd.min': 'Min', 'cd.sec': 'Seg',

      'alert.notVisible': 'Des de <b>{place}</b> aquest eclipsi <b>no és visible</b>. L\'ombra de la Lluna no passa per aquí.',
      'alert.total': '<b>Ets dins de la franja de totalitat!</b> Veuràs la corona solar durant <b>{dur}</b> a les <b>{time}</b>.',
      'alert.partial': 'Des d\'aquí l\'eclipsi serà <b>parcial</b> ({pct} % del Sol cobert). La línia central de totalitat passa a <b>{km} km</b> cap al <b>{dir}</b>. Val moltíssim la pena desplaçar-s\'hi: un 99 % parcial <i>no</i> s\'assembla gens a la totalitat.',
      'alert.belowHorizon': 'El màxim de l\'eclipsi passa <b>amb el Sol ja post</b> a la teva ubicació. Cal que et desplacis cap a l\'oest o cap al nord.',
      'alert.veryLow': 'El Sol estarà només a <b>{alt}°</b> d\'altura al màxim — poc més que l\'amplada de tres dits amb el braç estirat. Necessites un horitzó oest <b>completament net</b> (costa, mirador o plana).',
      'alert.low': 'El Sol estarà baix, a <b>{alt}°</b> d\'altura al màxim. Busca un lloc amb l\'horitzó oest lliure de muntanyes i edificis.',
      'alert.sunset': 'El Sol es pondrà <b>abans que acabi</b> l\'eclipsi: s\'amagarà encara mossegat per la Lluna. Un espectacle fotogràfic brutal.',

      'stat.type': 'Tipus d\'eclipsi',
      'stat.totality': 'Durada de la totalitat',
      'stat.mag': 'Magnitud màxima',
      'stat.obs': 'Sol cobert (àrea)',
      'stat.duration': 'Durada total de l\'esdeveniment',
      'stat.altMax': 'Altura del Sol al màxim',
      'stat.azMax': 'Azimut al màxim',
      'stat.ratio': 'Mida Lluna / Sol',
      'stat.noData': 'Sense dades.',
      'badge.total': 'Total', 'badge.annular': 'Anular', 'badge.partial': 'Parcial',

      'safety.1': '<b>No miris mai el Sol sense filtre homologat.</b> Necessites ulleres d\'eclipsi certificades <b>ISO 12312-2</b>. Les ulleres de sol, radiografies, CD o vidres fumats <b>no serveixen</b> i poden causar ceguesa permanent.',
      'safety.2': '<b>Només durant la totalitat</b> (entre C2 i C3) pots i has de mirar a ull nu: és l\'únic moment en què es veu la corona. Torna a posar-te el filtre <b>així que aparegui la primera espurna</b>.',
      'safety.3': 'Càmeres, prismàtics i telescopis necessiten <b>filtre solar davant de l\'objectiu</b>. Un filtre d\'ocular pot rebentar per la calor.',
      'safety.4': 'A la península l\'eclipsi passa amb el Sol <b>molt baix</b> sobre l\'horitzó oest. Busca un mirador net: un turó, la costa o una plana sense muntanyes a l\'oest. <b>Comprova l\'horitzó el dia abans a la mateixa hora.</b>',

      'tools.ics': '📅 Afegir al calendari',
      'tools.notify': '🔔 Avisos al mòbil',
      'tools.share': '🔗 Compartir',
      'tools.install': '⬇️ Instal·lar l\'app',
      'tools.notifyOn': '🔔 Avisos activats',
      'tools.notifyOnBody': 'T\'avisaré a cada fase de l\'eclipsi. Mantén l\'app oberta aquell dia.',
      'tools.notifyUnsupported': 'El teu navegador no admet notificacions.',
      'tools.copied': 'Enllaç copiat.',
      'tools.installHelp': 'A l\'iPhone: prem el botó «Compartir» del Safari i tria «Afegir a la pantalla d\'inici».\nA Android: menú ⋮ → «Instal·lar aplicació».',

      'ics.c1': '🌒 Comença l\'eclipsi solar (C1)',
      'ics.c1d': 'Posa\'t les ulleres d\'eclipsi ISO 12312-2. {place}',
      'ics.c2': '🌑 TOTALITAT de l\'eclipsi',
      'ics.c2d': 'Durada: {dur}. Treu-te el filtre NOMÉS ara.',
      'ics.max': '☀️ Màxim de l\'eclipsi',
      'ics.maxd': 'Magnitud {mag} · {pct} % cobert',
      'ics.c4': '🌘 Fi de l\'eclipsi (C4)',
      'ics.c4d': 'Últim contacte.',
      'share.total': 'Eclipsi total el 12/08/2026 des de {place}: totalitat a les {time} durant {dur}.',
      'share.other': 'Eclipsi solar del 12/08/2026 des de {place}.',

      'notif.c1': '🌒 Comença l\'eclipsi', 'notif.c1b': 'Primer contacte. Posa\'t el filtre solar.',
      'notif.c2pre': '⚡ Totalitat en 1 minut', 'notif.c2preb': 'Prepara\'t. Anell de diamant imminent.',
      'notif.c2': '🌑 TOTALITAT!', 'notif.c2b': 'Treu-te el filtre i mira la corona!',
      'notif.c3': '⚠️ Fi de la totalitat', 'notif.c3b': 'FILTRE POSAT JA!',
      'notif.c4': '🌘 Fi de l\'eclipsi', 'notif.c4b': 'Últim contacte. Fins al 2 d\'agost del 2027.',

      'dock.ar': 'Mode Realitat Augmentada',
      'ar.track': 'Trajectòria',
      'ar.disk': 'Disc solar',
      'ar.cal': '🎯 Calibrar amb el Sol',
      'ar.hintDefault': 'Apunta el mòbil cap on marca la fletxa',
      'ar.starting': 'Iniciant la càmera…',
      'ar.waitSensors': 'Esperant els sensors d\'orientació… mou una mica el mòbil.',
      'ar.moveCompass': 'Mou el mòbil per activar la brúixola.',
      'ar.relative': '⚠️ El teu mòbil no dóna <b>brúixola absoluta</b>: el rumb pot estar girat. Prem <b>Calibrar</b> i marca on és el Sol.',
      'ar.totality': '🌑 <b>TOTALITAT</b> — treu-te el filtre i mira directament',
      'ar.soon': '⚡ Totalitat imminent — <b>prepara\'t</b>',
      'ar.partial': '⛔ Eclipsi parcial en curs — <b>NO miris sense filtre ISO 12312-2</b>',
      'ar.belowHorizon': 'El Sol és sota l\'horitzó. La retícula marca on serà.',
      'ar.normal': 'La línia taronja és el recorregut del Sol durant l\'eclipsi. ⛔ No miris el Sol sense filtre.',
      'ar.calibHint': '🎯 Apunta amb la retícula al <b>Sol de veritat</b> i toca la pantalla. Si no el veus, fes servir una referència coneguda.',
      'ar.calibDone': '✅ Brúixola corregida <b>{off}°</b>. Es recorda per a la propera vegada.',
      'ar.calibCancel': 'Calibratge cancel·lat.',
      'ar.calibReset': 'Correcció de brúixola reiniciada.',
      'ar.noCamera': '⚠️ Sense accés a la càmera.<br>Només es mostra la superposició.',
      'ar.noSensors': 'Sense permís per als sensors d\'orientació. A l\'iPhone, recarrega i accepta l\'avís de «Moviment i orientació».',
      'ar.sun': '☀ SOL',
      'ar.sunBelow': '☀ SOL (sota l\'horitzó)',
      'ar.turnHere': 'Gira cap aquí',
      'ar.covered': 'Cobert',
      'ar.in': 'en',
      'ar.finished': 'Eclipsi acabat',
      'ar.relTag': 'brúixola relativa',

      'footer': 'Càlculs derivats dels <b>elements besselians de la NASA/GSFC</b> (F. Espenak), efemèrides VSOP87/ELP2000-85, ΔT = 71,4 s. Tot es calcula al teu dispositiu, sense servidor.<br>Verificat contra les dades de l\'<b>IGN</b> (Oviedo: magnitud 1,015, totalitat 1m 48s).<br><br>Fet amb estima per a qui fa dècades que espera aquest dia. ☀️🌑',

      'v.btnOff': '🔈 Activar la veu',
      'v.btnOn': '🔊 Veu activada',
      'v.btnBrief': '🗣️ Què veuré?',
      'v.ready': 'Veu activada. T\u2019aniré avisant a cada fase de l\u2019eclipsi.',
      'v.noVoice': 'El teu dispositiu no té cap veu instal·lada per a aquest idioma.',
      'v.fallback': 'El teu mòbil no té veu en català. Faré servir la castellana.',
      'v.briefWhere': 'Estàs a {place}.',
      'v.briefTotal': 'Des d\u2019aquí veuràs un eclipsi total. La totalitat comença a les {c2} i dura {dur}.',
      'v.briefPartial': 'Des d\u2019aquí l\u2019eclipsi serà parcial: la Lluna arribarà a tapar el {pct} per cent del Sol.',
      'v.briefTimes': 'Comença a les {c1} i acaba a les {c4}.',
      'v.briefTimesSet': 'Comença a les {c1}. L’eclipsi no s’acaba fins a les {c4}, però des d’aquí el Sol es pon abans, a les {set}: l’última part transcorre ja amb el Sol sota l’horitzó.',
      'v.briefSun': 'Has de mirar en direcció {dir}, i molt baix: el Sol estarà a només {alt} graus per damunt de l’horitzó, {hand}.',
      'v.briefFinger1': 'l’amplada d’un dit amb el braç estirat',
      'v.briefFingers': 'com {n} dits junts posant el braç estirat',
      'v.briefFist': 'com un puny tancat amb el braç estirat',
      'v.briefBlocked': 'Compte amb una cosa: des d’aquest punt exacte, a les {time} el Sol es posarà darrere del terreny i ja no el tornaràs a veure. Si vols l’eclipsi sencer, busca un lloc més obert cap a aquella banda.',
      'v.briefClear': 'Bona notícia: des d’aquí no hi ha cap muntanya que et tapi el Sol.',
      'v.briefBlockedLate': 'Una cosa més: veuràs la totalitat, però a les {time} el Sol es posarà darrere del terreny i et perdràs el tram final de l’eclipsi.',
      'v.briefCloudsBad': 'Ara mateix la previsió no acompanya: dóna un {low} per cent de núvols baixos a aquella hora, i són els que tapen.',
      'v.briefCloudsSome': 'La previsió dóna algun núvol baix, un {low} per cent, a aquella hora.',
      'v.briefCloudsGood': 'La previsió dóna el cel net a aquella hora.',
      'v.briefLow': 'Molt baix. Necessites l\u2019horitzó oest completament net.',
      'v.briefFilter': 'Recorda: filtre solar sempre posat, excepte durant la totalitat.',
      'v.briefFilterAlways': 'Recorda: aquí no hi ha totalitat. El filtre solar no te\'l pots treure en cap moment.',
      'cd.tap': '🔊 Toca el rellotge i t\u2019ho diré en veu alta',
      'v.greet': 'Falten {t} per a l\u2019inici de l\u2019eclipsi solar total.',
      'v.greetC2': 'L\u2019eclipsi ja ha començat. Falten {t} per a la totalitat.',
      'v.greetNow': 'Estàs en plena totalitat. Mira amunt!',
      'v.greetPartial': 'L\u2019eclipsi ja ha començat. Filtre posat.',
      'v.greetOver': 'L\u2019eclipsi del 12 d\u2019agost ja ha passat.',
      'v.greetNoEcl': 'Des d\u2019aquí l\u2019eclipsi no és visible.',
      'tl.voice': 'Que l\u2019app et parli',
      'tl.voiceOn': 'L\u2019app et parla',
      'tl.voiceDesc': 'T\u2019avisa a cada fase sense mirar el mòbil',
      'tl.brief': 'Què veuré?',
      'tl.briefDesc': 'Resum parlat',
      'tl.ics': 'Calendari',
      'tl.icsDesc': 'Amb recordatoris',
      'tl.notify': 'Avisos',
      'tl.notifyDesc': 'Notificacions',
      'tl.share': 'Compartir',
      'tl.shareDesc': 'Envia-ho',
      'tl.install': 'Instal·lar',
      'tl.installDesc': 'App al mòbil',
      'v.h1': 'Falta una hora per a l\u2019inici de l\u2019eclipsi. Bon moment per col·locar-te i comprovar que tens l\u2019horitzó oest lliure.',
      'v.m10': 'Deu minuts per a l\u2019inici. Posa\u2019t les ulleres d\u2019eclipsi ara.',
      'v.c1': 'Comença l\u2019eclipsi. La Lluna ja toca la vora del Sol. No miris sense filtre.',
      'v.p25': 'Un quart del Sol tapat. Mira l\u2019ombra dels arbres: cada forat entre les fulles projecta a terra una imatge del Sol en forma de mitja lluna.',
      'v.p50': 'Meitat del Sol tapada. La llum comença a canviar: els colors es tornen més plans i les ombres, més nítides.',
      'v.p75': 'Tres quartes parts tapades. Notaràs que baixa la temperatura i que els ocells es queden en silenci.',
      'v.m5': 'Cinc minuts per a la totalitat. Mira cap a l\u2019oest: veuràs arribar l\u2019ombra de la Lluna com una paret fosca.',
      'v.m1': 'Un minut per a la totalitat. Prepara\u2019t per treure\u2019t el filtre quan t\u2019avisi.',
      'v.s20': 'Vint segons. Mira una superfície clara a terra: potser hi veuràs bandes d\u2019ombra tremoloses.',
      'v.c2': 'Totalitat! Treu-te les ulleres. Ja pots mirar el Sol directament.',
      'v.mid': 'Estàs veient la corona solar. Busca Venus, brillant, a prop del Sol. I mira l\u2019horitzó: tens una posta de sol de tres-cents seixanta graus al teu voltant.',
      'v.c3warn': 'Atenció. Ulleres posades, ara.',
      'v.c3': 'S\u2019ha acabat la totalitat. Filtre posat fins al final.',
      'v.c4': 'Fi de l\u2019eclipsi. Fins al 2 d\u2019agost del 2027.',
      'v.pMax': 'Màxim de l\u2019eclipsi. El {pct} per cent del Sol està tapat. No et treguis el filtre en cap moment.',
      'v.dirSpoken': ['nord','nord-nord-est','nord-est','est-nord-est','est','est-sud-est','sud-est','sud-sud-est','sud','sud-sud-oest','sud-oest','oest-sud-oest','oest','oest-nord-oest','nord-oest','nord-nord-oest'],
      'dir': ['N','NNE','NE','ENE','E','ESE','SE','SSE','S','SSO','SO','OSO','O','ONO','NO','NNO'],
      'mark.totality': 'Total',
      'mark.c1': 'Inici',
      'mark.c2': 'Total',
      'mark.max': 'Màxim',
      'mark.c3': 'Fi',
      'mark.c4': 'Final'
    },

    /* ================= CASTELLANO ================= */
    es: {
      'html.lang': 'es',
      'html.dir': 'ltr',
      'locale': 'es-ES',
      'doc.title': 'Eclipse AR 2026 · 12 de agosto',

      'app.kicker': 'Eclipse Solar Total',
      'app.title': '12 de agosto de 2026',
      'app.subtitle': 'El primer eclipse total visible desde la península ibérica en más de un siglo',
      'app.geo': 'Usar mi ubicación GPS',
      'app.geoActive': 'Ubicación GPS activa',
      'app.geoLocating': 'Localizando…',
      'app.geoError': 'No se pudo obtener la ubicación: ',
      'app.geoHttps': 'Recuerda que el GPS requiere HTTPS.',
      'app.geoUnsupported': 'Tu navegador no soporta geolocalización.',
      'app.myLocation': 'Mi ubicación',
      'app.mapPoint': 'Punto del mapa',

      'card.phases': 'Fases del eclipse en tu posición',
      'card.sim': 'Simulación en tiempo real',
      'card.sun': 'Dónde está el Sol ahora mismo',
      'card.local': 'Circunstancias locales',
      'card.band': 'Franja de totalidad',
      'card.cities': 'Ciudades de referencia',
      'card.safety': 'Seguridad y consejos',
      'card.tools': 'Herramientas',

      'ro.mag': 'Magnitud',
      'ro.obs': 'Sol cubierto',
      'ro.phase': 'Fase',
      'ro.total': 'TOTAL',
      'ro.partial': 'Parcial',
      'ro.before': 'Aún no',
      'ro.after': 'Terminado',

      'scrub.explore': 'Explorar el eclipse',
      'scrub.live': 'EN VIVO',
      'scrub.drag': 'Arrastra para adelantar / retroceder',
      'scrub.back': 'Volver a directo',

      'sun.az': 'Azimut (desde el Norte)',
      'sun.alt': 'Altura sobre el horizonte',
      'sun.dir': 'Orientación',
      'sun.set': 'Ocaso hoy',
      'sun.below': '(bajo el horizonte)',
      'sun.note': 'La brújula muestra el <b>Norte arriba</b>; el borde es el horizonte y el centro, el cenit. La línea naranja es el recorrido que hará el Sol durante el eclipse, y el punto blanco marca el instante del máximo.',

      'legend.center': 'Línea central',
      'legend.band': 'Banda de totalidad',
      'legend.you': 'Tu posición',
      'map.note': 'La línea central más próxima está a <b>{km} km</b> hacia el <b>{dir}</b> ({lat}°, {lon}°). Toca el mapa para calcular cualquier otro punto.',
      'map.noteSimple': 'Toca el mapa para calcular las circunstancias en cualquier punto.',
      'map.offline': 'Mapa no disponible sin conexión.',

      'cities.note': 'En <b>naranja</b>, las que están dentro de la franja de totalidad.',

      'phases.note': 'Horas en tu zona horaria local. «Alt» es la altura del Sol sobre el horizonte en ese instante.',
      'phases.noTotality': 'Sin fase de totalidad en esta ubicación: el eclipse será parcial y <b>nunca</b> podrás quitarte el filtro.',

      'phase.c1': 'C1 · Primer contacto',
      'phase.c1d': 'La Luna toca el borde del Sol. Empieza el eclipse parcial. <b>Filtro puesto.</b>',
      'phase.c2': 'C2 · Inicio de la totalidad',
      'phase.c2d': 'Anillo de diamante y perlas de Baily. <b>Ya puedes quitarte el filtro.</b>',
      'phase.max': 'Máximo del eclipse',
      'phase.maxd': 'Momento álgido. Corona solar, planetas y estrellas visibles.',
      'phase.c3': 'C3 · Fin de la totalidad',
      'phase.c3d': 'Reaparece el Sol. <b>Filtro puesto YA</b>, antes del primer destello.',
      'phase.c4': 'C4 · Último contacto',
      'phase.c4d': 'La Luna abandona el disco solar. Fin del eclipse.',

      'cd.c1': 'Faltan para el INICIO (C1)',
      'cd.c2': '⚡ Faltan para la TOTALIDAD (C2)',
      'cd.max': 'Faltan para el MÁXIMO',
      'cd.c3': '🔴 Queda de TOTALIDAD — ¡mira ahora!',
      'cd.c4': 'Faltan para el FINAL (C4)',
      'cd.done': '✨ El eclipse ha terminado',
      'cd.notVisible': 'Eclipse no visible desde aquí',
      'cd.days': 'Días', 'cd.hours': 'Horas', 'cd.min': 'Min', 'cd.sec': 'Seg',

      'alert.notVisible': 'Desde <b>{place}</b> este eclipse <b>no es visible</b>. La sombra de la Luna no pasa por aquí.',
      'alert.total': '<b>¡Estás dentro de la franja de totalidad!</b> Verás la corona solar durante <b>{dur}</b> a las <b>{time}</b>.',
      'alert.partial': 'Desde aquí el eclipse será <b>parcial</b> ({pct} % del Sol cubierto). La línea central de totalidad pasa a <b>{km} km</b> hacia el <b>{dir}</b>. Merece muchísimo la pena desplazarse: un 99 % parcial <i>no</i> se parece en nada a la totalidad.',
      'alert.belowHorizon': 'El máximo del eclipse ocurre <b>con el Sol ya puesto</b> en tu ubicación. Necesitas desplazarte al oeste o al norte.',
      'alert.veryLow': 'El Sol estará a solo <b>{alt}°</b> de altura en el máximo — poco más que el ancho de tres dedos con el brazo extendido. Necesitas un horizonte oeste <b>completamente despejado</b> (costa, mirador o llanura).',
      'alert.low': 'El Sol estará bajo, a <b>{alt}°</b> de altura en el máximo. Busca un lugar con el horizonte oeste libre de montañas y edificios.',
      'alert.sunset': 'El Sol se pondrá <b>antes de que acabe</b> el eclipse: se ocultará todavía mordido por la Luna. Un espectáculo fotográfico brutal.',

      'stat.type': 'Tipo de eclipse',
      'stat.totality': 'Duración de la totalidad',
      'stat.mag': 'Magnitud máxima',
      'stat.obs': 'Sol cubierto (área)',
      'stat.duration': 'Duración total del evento',
      'stat.altMax': 'Altura del Sol en el máximo',
      'stat.azMax': 'Azimut en el máximo',
      'stat.ratio': 'Tamaño Luna / Sol',
      'stat.noData': 'Sin datos.',
      'badge.total': 'Total', 'badge.annular': 'Anular', 'badge.partial': 'Parcial',

      'safety.1': '<b>Nunca mires al Sol sin filtro homologado.</b> Necesitas gafas de eclipse certificadas <b>ISO 12312-2</b>. Las gafas de sol, radiografías, CDs o cristales ahumados <b>no sirven</b> y pueden causar ceguera permanente.',
      'safety.2': '<b>Solo durante la totalidad</b> (entre C2 y C3) puedes y debes mirar a simple vista: es el único momento en que se ve la corona. Vuelve a ponerte el filtro <b>en cuanto asome el primer destello</b>.',
      'safety.3': 'Cámaras, prismáticos y telescopios necesitan <b>filtro solar delante del objetivo</b>. Un filtro de ocular puede reventar por el calor.',
      'safety.4': 'En la península el eclipse ocurre con el Sol <b>muy bajo</b> sobre el horizonte oeste. Busca un mirador despejado: una colina, la costa o un llano sin montañas al oeste. <b>Comprueba el horizonte el día antes a la misma hora.</b>',

      'tools.ics': '📅 Añadir al calendario',
      'tools.notify': '🔔 Avisos en el móvil',
      'tools.share': '🔗 Compartir',
      'tools.install': '⬇️ Instalar la app',
      'tools.notifyOn': '🔔 Avisos activados',
      'tools.notifyOnBody': 'Te avisaré en cada fase del eclipse. Mantén la app abierta ese día.',
      'tools.notifyUnsupported': 'Tu navegador no soporta notificaciones.',
      'tools.copied': 'Enlace copiado.',
      'tools.installHelp': 'En iPhone: pulsa el botón «Compartir» de Safari y elige «Añadir a pantalla de inicio».\nEn Android: menú ⋮ → «Instalar aplicación».',

      'ics.c1': '🌒 Empieza el eclipse solar (C1)',
      'ics.c1d': 'Ponte las gafas de eclipse ISO 12312-2. {place}',
      'ics.c2': '🌑 TOTALIDAD del eclipse',
      'ics.c2d': 'Duración: {dur}. Quítate el filtro SOLO ahora.',
      'ics.max': '☀️ Máximo del eclipse',
      'ics.maxd': 'Magnitud {mag} · {pct} % cubierto',
      'ics.c4': '🌘 Fin del eclipse (C4)',
      'ics.c4d': 'Último contacto.',
      'share.total': 'Eclipse total el 12/08/2026 desde {place}: totalidad a las {time} durante {dur}.',
      'share.other': 'Eclipse solar del 12/08/2026 desde {place}.',

      'notif.c1': '🌒 Empieza el eclipse', 'notif.c1b': 'Primer contacto. Ponte el filtro solar.',
      'notif.c2pre': '⚡ Totalidad en 1 minuto', 'notif.c2preb': 'Prepárate. Anillo de diamante inminente.',
      'notif.c2': '🌑 ¡TOTALIDAD!', 'notif.c2b': '¡Quítate el filtro y mira la corona!',
      'notif.c3': '⚠️ Fin de la totalidad', 'notif.c3b': '¡FILTRO PUESTO YA!',
      'notif.c4': '🌘 Fin del eclipse', 'notif.c4b': 'Último contacto. Hasta el 2 de agosto de 2027.',

      'dock.ar': 'Modo Realidad Aumentada',
      'ar.track': 'Trayectoria',
      'ar.disk': 'Disco solar',
      'ar.cal': '🎯 Calibrar con el Sol',
      'ar.hintDefault': 'Apunta el móvil hacia donde marca la flecha',
      'ar.starting': 'Iniciando cámara…',
      'ar.waitSensors': 'Esperando a los sensores de orientación… mueve un poco el móvil.',
      'ar.moveCompass': 'Mueve el móvil para activar la brújula.',
      'ar.relative': '⚠️ Tu móvil no da <b>brújula absoluta</b>: el rumbo puede estar girado. Pulsa <b>Calibrar</b> y marca dónde está el Sol.',
      'ar.totality': '🌑 <b>TOTALIDAD</b> — quítate el filtro y mira directamente',
      'ar.soon': '⚡ Totalidad inminente — <b>prepárate</b>',
      'ar.partial': '⛔ Eclipse parcial en curso — <b>NO mires sin filtro ISO 12312-2</b>',
      'ar.belowHorizon': 'El Sol está bajo el horizonte. La retícula marca dónde estará.',
      'ar.normal': 'La línea naranja es el recorrido del Sol durante el eclipse. ⛔ No mires al Sol sin filtro.',
      'ar.calibHint': '🎯 Apunta con la retícula al <b>Sol de verdad</b> y toca la pantalla. Si no lo ves, usa una referencia conocida.',
      'ar.calibDone': '✅ Brújula corregida <b>{off}°</b>. Se recuerda para la próxima vez.',
      'ar.calibCancel': 'Calibración cancelada.',
      'ar.calibReset': 'Corrección de brújula reiniciada.',
      'ar.noCamera': '⚠️ Sin acceso a la cámara.<br>Se muestra solo la superposición.',
      'ar.noSensors': 'Sin permiso para los sensores de orientación. En iPhone, recarga y acepta el aviso de «Movimiento y orientación».',
      'ar.sun': '☀ SOL',
      'ar.sunBelow': '☀ SOL (bajo el horizonte)',
      'ar.turnHere': 'Gira hacia aquí',
      'ar.covered': 'Cubierto',
      'ar.in': 'en',
      'ar.finished': 'Eclipse terminado',
      'ar.relTag': 'brújula relativa',

      'footer': 'Cálculos derivados de los <b>elementos besselianos de la NASA/GSFC</b> (F. Espenak), efemérides VSOP87/ELP2000-85, ΔT = 71,4 s. Todo se calcula en tu dispositivo, sin servidor.<br>Verificado contra los datos del <b>IGN</b> (Oviedo: magnitud 1,015, totalidad 1m 48s).<br><br>Hecho con cariño para los que llevan décadas esperando este día. ☀️🌑',

      'v.btnOff': '🔈 Activar la voz',
      'v.btnOn': '🔊 Voz activada',
      'v.btnBrief': '🗣️ ¿Qué voy a ver?',
      'v.ready': 'Voz activada. Te iré avisando en cada fase del eclipse.',
      'v.noVoice': 'Tu dispositivo no tiene ninguna voz instalada para este idioma.',
      'v.fallback': 'Tu móvil no tiene voz en catalán. Usaré la castellana.',
      'v.briefWhere': 'Estás en {place}.',
      'v.briefTotal': 'Desde aquí verás un eclipse total. La totalidad empieza a las {c2} y dura {dur}.',
      'v.briefPartial': 'Desde aquí el eclipse será parcial: la Luna llegará a tapar el {pct} por ciento del Sol.',
      'v.briefTimes': 'Empieza a las {c1} y termina a las {c4}.',
      'v.briefTimesSet': 'Empieza a las {c1}. El eclipse no acaba hasta las {c4}, pero desde aquí el Sol se pone antes, a las {set}: la última parte transcurre ya con el Sol por debajo del horizonte.',
      'v.briefSun': 'Tienes que mirar en dirección {dir}, y muy bajo: el Sol estará a solo {alt} grados por encima del horizonte, {hand}.',
      'v.briefFinger1': 'lo que ocupa un dedo con el brazo estirado',
      'v.briefFingers': 'como {n} dedos juntos poniendo el brazo estirado',
      'v.briefFist': 'como un puño cerrado con el brazo estirado',
      'v.briefBlocked': 'Ojo con una cosa: desde este punto exacto, a las {time} el Sol se meterá detrás del terreno y ya no lo volverás a ver. Si quieres el eclipse entero, busca un sitio más despejado hacia ese lado.',
      'v.briefClear': 'Buena noticia: desde aquí no hay ninguna montaña que te tape el Sol.',
      'v.briefBlockedLate': 'Una cosa más: verás la totalidad, pero a las {time} el Sol se meterá detrás del terreno y te perderás el tramo final del eclipse.',
      'v.briefCloudsBad': 'Ahora mismo la previsión no acompaña: da un {low} por ciento de nubes bajas a esa hora, que son las que tapan.',
      'v.briefCloudsSome': 'La previsión da alguna nube baja, un {low} por ciento, a esa hora.',
      'v.briefCloudsGood': 'La previsión da el cielo despejado a esa hora.',
      'v.briefLow': 'Muy bajo. Necesitas el horizonte oeste completamente despejado.',
      'v.briefFilter': 'Recuerda: filtro solar siempre puesto, salvo durante la totalidad.',
      'v.briefFilterAlways': 'Recuerda: aquí no hay totalidad. El filtro solar no te lo puedes quitar en ningún momento.',
      'cd.tap': '🔊 Toca el reloj y te lo digo en voz alta',
      'v.greet': 'Faltan {t} para el inicio del eclipse solar total.',
      'v.greetC2': 'El eclipse ya ha empezado. Faltan {t} para la totalidad.',
      'v.greetNow': '¡Estás en plena totalidad. Mira arriba!',
      'v.greetPartial': 'El eclipse ya ha empezado. Filtro puesto.',
      'v.greetOver': 'El eclipse del 12 de agosto ya ha pasado.',
      'v.greetNoEcl': 'Desde aquí el eclipse no es visible.',
      'tl.voice': 'Que la app te hable',
      'tl.voiceOn': 'La app te habla',
      'tl.voiceDesc': 'Te avisa en cada fase sin mirar el móvil',
      'tl.brief': '¿Qué veré?',
      'tl.briefDesc': 'Resumen hablado',
      'tl.ics': 'Calendario',
      'tl.icsDesc': 'Con recordatorios',
      'tl.notify': 'Avisos',
      'tl.notifyDesc': 'Notificaciones',
      'tl.share': 'Compartir',
      'tl.shareDesc': 'Envíalo',
      'tl.install': 'Instalar',
      'tl.installDesc': 'App en el móvil',
      'v.h1': 'Falta una hora para el inicio del eclipse. Buen momento para colocarte y comprobar que tienes el horizonte oeste libre.',
      'v.m10': 'Diez minutos para el inicio. Ponte las gafas de eclipse ahora.',
      'v.c1': 'Empieza el eclipse. La Luna ya toca el borde del Sol. No mires sin filtro.',
      'v.p25': 'Un cuarto del Sol tapado. Mira la sombra de los árboles: cada hueco entre las hojas proyecta en el suelo una imagen del Sol en forma de media luna.',
      'v.p50': 'Mitad del Sol tapada. La luz empieza a cambiar: los colores se vuelven más planos y las sombras, más nítidas.',
      'v.p75': 'Tres cuartas partes tapadas. Notarás que baja la temperatura y que los pájaros se quedan en silencio.',
      'v.m5': 'Cinco minutos para la totalidad. Mira hacia el oeste: verás llegar la sombra de la Luna como una pared oscura.',
      'v.m1': 'Un minuto para la totalidad. Prepárate para quitarte el filtro cuando te avise.',
      'v.s20': 'Veinte segundos. Mira una superficie clara en el suelo: puede que veas bandas de sombra temblorosas.',
      'v.c2': '¡Totalidad! Quítate las gafas. Ya puedes mirar el Sol directamente.',
      'v.mid': 'Estás viendo la corona solar. Busca a Venus, brillante, cerca del Sol. Y mira al horizonte: tienes un atardecer de trescientos sesenta grados a tu alrededor.',
      'v.c3warn': 'Atención. Gafas puestas, ahora.',
      'v.c3': 'Se acabó la totalidad. Filtro puesto hasta el final.',
      'v.c4': 'Fin del eclipse. Hasta el 2 de agosto de 2027.',
      'v.pMax': 'Máximo del eclipse. El {pct} por ciento del Sol está tapado. No te quites el filtro en ningún momento.',
      'v.dirSpoken': ['norte','nor-noreste','noreste','este-noreste','este','este-sureste','sureste','sur-sureste','sur','sur-suroeste','suroeste','oeste-suroeste','oeste','oeste-noroeste','noroeste','nor-noroeste'],
      'dir': ['N','NNE','NE','ENE','E','ESE','SE','SSE','S','SSO','SO','OSO','O','ONO','NO','NNO'],
      'mark.totality': 'Total',
      'mark.c1': 'Inicio',
      'mark.c2': 'Total',
      'mark.max': 'Máximo',
      'mark.c3': 'Fin',
      'mark.c4': 'Final'
    },

    /* ================= ENGLISH ================= */
    en: {
      'html.lang': 'en',
      'html.dir': 'ltr',
      'locale': 'en-GB',
      'doc.title': 'Eclipse AR 2026 · 12 August',

      'app.kicker': 'Total Solar Eclipse',
      'app.title': '12 August 2026',
      'app.subtitle': 'The first total eclipse visible from the Iberian Peninsula in over a century',
      'app.geo': 'Use my GPS location',
      'app.geoActive': 'GPS location active',
      'app.geoLocating': 'Locating…',
      'app.geoError': 'Could not get your location: ',
      'app.geoHttps': 'Remember that GPS requires HTTPS.',
      'app.geoUnsupported': 'Your browser does not support geolocation.',
      'app.myLocation': 'My location',
      'app.mapPoint': 'Map point',

      'card.phases': 'Eclipse phases at your location',
      'card.sim': 'Live simulation',
      'card.sun': 'Where the Sun is right now',
      'card.local': 'Local circumstances',
      'card.band': 'Path of totality',
      'card.cities': 'Reference cities',
      'card.safety': 'Safety and tips',
      'card.tools': 'Tools',

      'ro.mag': 'Magnitude',
      'ro.obs': 'Sun covered',
      'ro.phase': 'Phase',
      'ro.total': 'TOTAL',
      'ro.partial': 'Partial',
      'ro.before': 'Not yet',
      'ro.after': 'Over',

      'scrub.explore': 'Explore the eclipse',
      'scrub.live': 'LIVE',
      'scrub.drag': 'Drag to skip forward / back',
      'scrub.back': 'Back to live',

      'sun.az': 'Azimuth (from North)',
      'sun.alt': 'Altitude above horizon',
      'sun.dir': 'Direction',
      'sun.set': 'Sunset today',
      'sun.below': '(below the horizon)',
      'sun.note': 'The compass shows <b>North at the top</b>; the rim is the horizon and the centre is the zenith. The orange line is the path the Sun will follow during the eclipse, and the white dot marks the moment of maximum.',

      'legend.center': 'Central line',
      'legend.band': 'Path of totality',
      'legend.you': 'Your position',
      'map.note': 'The nearest central line is <b>{km} km</b> to the <b>{dir}</b> ({lat}°, {lon}°). Tap the map to compute any other point.',
      'map.noteSimple': 'Tap the map to compute circumstances anywhere.',
      'map.offline': 'Map unavailable offline.',

      'cities.note': 'In <b>orange</b>, those inside the path of totality.',

      'phases.note': 'Times in your local time zone. «Alt» is the Sun\'s altitude above the horizon at that instant.',
      'phases.noTotality': 'No totality at this location: the eclipse will be partial and you must <b>never</b> remove your filter.',

      'phase.c1': 'C1 · First contact',
      'phase.c1d': 'The Moon touches the Sun\'s edge. The partial eclipse begins. <b>Filter on.</b>',
      'phase.c2': 'C2 · Totality begins',
      'phase.c2d': 'Diamond ring and Baily\'s beads. <b>You may now remove the filter.</b>',
      'phase.max': 'Maximum eclipse',
      'phase.maxd': 'The peak. Solar corona, planets and stars visible.',
      'phase.c3': 'C3 · Totality ends',
      'phase.c3d': 'The Sun reappears. <b>Filter back on NOW</b>, before the first flash.',
      'phase.c4': 'C4 · Last contact',
      'phase.c4d': 'The Moon leaves the solar disc. End of the eclipse.',

      'cd.c1': 'Until the START (C1)',
      'cd.c2': '⚡ Until TOTALITY (C2)',
      'cd.max': 'Until MAXIMUM',
      'cd.c3': '🔴 TOTALITY remaining — look now!',
      'cd.c4': 'Until the END (C4)',
      'cd.done': '✨ The eclipse is over',
      'cd.notVisible': 'Eclipse not visible from here',
      'cd.days': 'Days', 'cd.hours': 'Hours', 'cd.min': 'Min', 'cd.sec': 'Sec',

      'alert.notVisible': 'From <b>{place}</b> this eclipse is <b>not visible</b>. The Moon\'s shadow does not pass here.',
      'alert.total': '<b>You are inside the path of totality!</b> You will see the solar corona for <b>{dur}</b> at <b>{time}</b>.',
      'alert.partial': 'From here the eclipse will be <b>partial</b> ({pct} % of the Sun covered). The central line of totality passes <b>{km} km</b> to the <b>{dir}</b>. It is absolutely worth travelling: a 99 % partial eclipse is <i>nothing</i> like totality.',
      'alert.belowHorizon': 'Maximum eclipse happens <b>after sunset</b> at your location. You need to move west or north.',
      'alert.veryLow': 'The Sun will be only <b>{alt}°</b> high at maximum — barely three finger widths at arm\'s length. You need a <b>completely clear</b> western horizon (coast, viewpoint or open plain).',
      'alert.low': 'The Sun will be low, <b>{alt}°</b> high at maximum. Find a spot with the western horizon free of mountains and buildings.',
      'alert.sunset': 'The Sun will set <b>before the eclipse ends</b>: it will disappear still bitten by the Moon. A spectacular photo opportunity.',

      'stat.type': 'Eclipse type',
      'stat.totality': 'Duration of totality',
      'stat.mag': 'Maximum magnitude',
      'stat.obs': 'Sun covered (area)',
      'stat.duration': 'Total event duration',
      'stat.altMax': 'Sun altitude at maximum',
      'stat.azMax': 'Azimuth at maximum',
      'stat.ratio': 'Moon / Sun size',
      'stat.noData': 'No data.',
      'badge.total': 'Total', 'badge.annular': 'Annular', 'badge.partial': 'Partial',

      'safety.1': '<b>Never look at the Sun without a certified filter.</b> You need <b>ISO 12312-2</b> certified eclipse glasses. Sunglasses, X-ray film, CDs or smoked glass <b>do not work</b> and can cause permanent blindness.',
      'safety.2': '<b>Only during totality</b> (between C2 and C3) can and should you look with the naked eye: it is the only moment the corona is visible. Put the filter back on <b>the instant the first flash appears</b>.',
      'safety.3': 'Cameras, binoculars and telescopes need a <b>solar filter in front of the objective</b>. An eyepiece filter can shatter from the heat.',
      'safety.4': 'In Iberia the eclipse happens with the Sun <b>very low</b> over the western horizon. Find a clear viewpoint: a hill, the coast or an open plain with no mountains to the west. <b>Check your horizon the day before at the same time.</b>',

      'tools.ics': '📅 Add to calendar',
      'tools.notify': '🔔 Phone alerts',
      'tools.share': '🔗 Share',
      'tools.install': '⬇️ Install the app',
      'tools.notifyOn': '🔔 Alerts enabled',
      'tools.notifyOnBody': 'I will alert you at every eclipse phase. Keep the app open that day.',
      'tools.notifyUnsupported': 'Your browser does not support notifications.',
      'tools.copied': 'Link copied.',
      'tools.installHelp': 'On iPhone: tap Safari\'s «Share» button and choose «Add to Home Screen».\nOn Android: menu ⋮ → «Install app».',

      'ics.c1': '🌒 Solar eclipse begins (C1)',
      'ics.c1d': 'Put on your ISO 12312-2 eclipse glasses. {place}',
      'ics.c2': '🌑 TOTALITY',
      'ics.c2d': 'Duration: {dur}. Remove the filter ONLY now.',
      'ics.max': '☀️ Maximum eclipse',
      'ics.maxd': 'Magnitude {mag} · {pct} % covered',
      'ics.c4': '🌘 Eclipse ends (C4)',
      'ics.c4d': 'Last contact.',
      'share.total': 'Total eclipse on 12/08/2026 from {place}: totality at {time} lasting {dur}.',
      'share.other': 'Solar eclipse of 12/08/2026 from {place}.',

      'notif.c1': '🌒 Eclipse begins', 'notif.c1b': 'First contact. Put on your solar filter.',
      'notif.c2pre': '⚡ Totality in 1 minute', 'notif.c2preb': 'Get ready. Diamond ring imminent.',
      'notif.c2': '🌑 TOTALITY!', 'notif.c2b': 'Remove the filter and look at the corona!',
      'notif.c3': '⚠️ Totality ending', 'notif.c3b': 'FILTER BACK ON NOW!',
      'notif.c4': '🌘 Eclipse over', 'notif.c4b': 'Last contact. See you on 2 August 2027.',

      'dock.ar': 'Augmented Reality mode',
      'ar.track': 'Path',
      'ar.disk': 'Solar disc',
      'ar.cal': '🎯 Calibrate on the Sun',
      'ar.hintDefault': 'Point your phone where the arrow shows',
      'ar.starting': 'Starting camera…',
      'ar.waitSensors': 'Waiting for orientation sensors… move the phone a little.',
      'ar.moveCompass': 'Move the phone to wake the compass.',
      'ar.relative': '⚠️ Your phone has no <b>absolute compass</b>: the heading may be rotated. Tap <b>Calibrate</b> and mark where the Sun is.',
      'ar.totality': '🌑 <b>TOTALITY</b> — remove the filter and look directly',
      'ar.soon': '⚡ Totality imminent — <b>get ready</b>',
      'ar.partial': '⛔ Partial eclipse in progress — <b>do NOT look without an ISO 12312-2 filter</b>',
      'ar.belowHorizon': 'The Sun is below the horizon. The reticle marks where it will be.',
      'ar.normal': 'The orange line is the Sun\'s path during the eclipse. ⛔ Never look at the Sun without a filter.',
      'ar.calibHint': '🎯 Aim the reticle at the <b>real Sun</b> and tap the screen. If you cannot see it, use a known landmark.',
      'ar.calibDone': '✅ Compass corrected by <b>{off}°</b>. Remembered for next time.',
      'ar.calibCancel': 'Calibration cancelled.',
      'ar.calibReset': 'Compass correction reset.',
      'ar.noCamera': '⚠️ No camera access.<br>Showing the overlay only.',
      'ar.noSensors': 'No permission for orientation sensors. On iPhone, reload and accept the «Motion & Orientation» prompt.',
      'ar.sun': '☀ SUN',
      'ar.sunBelow': '☀ SUN (below horizon)',
      'ar.turnHere': 'Turn this way',
      'ar.covered': 'Covered',
      'ar.in': 'in',
      'ar.finished': 'Eclipse over',
      'ar.relTag': 'relative compass',

      'footer': 'Calculations derived from <b>NASA/GSFC Besselian elements</b> (F. Espenak), VSOP87/ELP2000-85 ephemerides, ΔT = 71.4 s. Everything is computed on your device, with no server.<br>Verified against <b>IGN</b> data (Oviedo: magnitude 1.015, totality 1m 48s).<br><br>Made with care for everyone who has waited decades for this day. ☀️🌑',

      'v.btnOff': '🔈 Turn on voice',
      'v.btnOn': '🔊 Voice on',
      'v.btnBrief': '🗣️ What will I see?',
      'v.ready': 'Voice enabled. I will guide you through every phase of the eclipse.',
      'v.noVoice': 'Your device has no voice installed for this language.',
      'v.fallback': 'Your phone has no Catalan voice. I will use the Spanish one.',
      'v.briefWhere': 'You are at {place}.',
      'v.briefTotal': 'From here you will see a total eclipse. Totality begins at {c2} and lasts {dur}.',
      'v.briefPartial': 'From here the eclipse will be partial: the Moon will cover up to {pct} per cent of the Sun.',
      'v.briefTimes': 'It starts at {c1} and ends at {c4}.',
      'v.briefTimesSet': 'It starts at {c1}. The eclipse does not end until {c4}, but from here the Sun sets earlier, at {set}: the last part happens with the Sun already below the horizon.',
      'v.briefSun': 'You need to look towards the {dir}, and very low: the Sun will be only {alt} degrees above the horizon, {hand}.',
      'v.briefFinger1': 'about one finger wide at arm’s length',
      'v.briefFingers': 'about {n} fingers together at arm’s length',
      'v.briefFist': 'about a closed fist at arm’s length',
      'v.briefBlocked': 'One thing to watch: from this exact spot, at {time} the Sun will drop behind the ground and you will not see it again. If you want the whole eclipse, find somewhere more open towards that side.',
      'v.briefClear': 'Good news: from here there is no hill in the way of the Sun.',
      'v.briefBlockedLate': 'One more thing: you will see totality, but at {time} the Sun drops behind the ground and you will miss the last part of the eclipse.',
      'v.briefCloudsBad': 'Right now the forecast is not helping: {low} per cent low cloud at that hour, and low cloud is what hides the Sun.',
      'v.briefCloudsSome': 'The forecast gives some low cloud, {low} per cent, at that hour.',
      'v.briefCloudsGood': 'The forecast gives clear sky at that hour.',
      'v.briefLow': 'Very low. You need a completely clear western horizon.',
      'v.briefFilter': 'Remember: keep your solar filter on at all times, except during totality.',
      'v.briefFilterAlways': 'Remember: there is no totality here. You must never remove your solar filter.',
      'cd.tap': '🔊 Tap the clock and I\u2019ll say it out loud',
      'v.greet': '{t} to go until the total solar eclipse begins.',
      'v.greetC2': 'The eclipse has already begun. {t} to totality.',
      'v.greetNow': 'You are in totality right now. Look up!',
      'v.greetPartial': 'The eclipse has already begun. Filter on.',
      'v.greetOver': 'The eclipse of the twelfth of August has already passed.',
      'v.greetNoEcl': 'The eclipse is not visible from here.',
      'tl.voice': 'Let the app talk',
      'tl.voiceOn': 'The app is talking',
      'tl.voiceDesc': 'Guides you through every phase, hands free',
      'tl.brief': 'Briefing',
      'tl.briefDesc': 'Spoken summary',
      'tl.ics': 'Calendar',
      'tl.icsDesc': 'With reminders',
      'tl.notify': 'Alerts',
      'tl.notifyDesc': 'Notifications',
      'tl.share': 'Share',
      'tl.shareDesc': 'Send it',
      'tl.install': 'Install',
      'tl.installDesc': 'App on your phone',
      'v.h1': 'One hour to the start of the eclipse. Good time to settle in and check that your western horizon is clear.',
      'v.m10': 'Ten minutes to first contact. Put your eclipse glasses on now.',
      'v.c1': 'The eclipse has begun. The Moon is touching the edge of the Sun. Do not look without a filter.',
      'v.p25': 'A quarter of the Sun is covered. Look at the shadows under the trees: every gap between the leaves projects a crescent-shaped image of the Sun on the ground.',
      'v.p50': 'Half the Sun is covered. The light is starting to change: colours flatten and shadows grow sharper.',
      'v.p75': 'Three quarters covered. You will feel the temperature drop and hear the birds fall silent.',
      'v.m5': 'Five minutes to totality. Look west: you will see the Moon\u2019s shadow arriving like a dark wall.',
      'v.m1': 'One minute to totality. Get ready to remove your filter when I tell you.',
      'v.s20': 'Twenty seconds. Watch a pale surface on the ground: you may see shimmering shadow bands.',
      'v.c2': 'Totality! Glasses off. You can look straight at the Sun now.',
      'v.mid': 'You are looking at the solar corona. Find Venus, shining close to the Sun. And look at the horizon: you have a three hundred and sixty degree sunset all around you.',
      'v.c3warn': 'Warning. Glasses back on, now.',
      'v.c3': 'Totality is over. Keep the filter on until the end.',
      'v.c4': 'The eclipse is over. See you on the second of August, 2027.',
      'v.pMax': 'Maximum eclipse. {pct} per cent of the Sun is covered. Do not remove your filter at any point.',
      'v.dirSpoken': ['north','north-northeast','northeast','east-northeast','east','east-southeast','southeast','south-southeast','south','south-southwest','southwest','west-southwest','west','west-northwest','northwest','north-northwest'],
      'dir': ['N','NNE','NE','ENE','E','ESE','SE','SSE','S','SSW','SW','WSW','W','WNW','NW','NNW'],
      'mark.totality': 'Total',
      'mark.c1': 'Start',
      'mark.c2': 'Total',
      'mark.max': 'Max',
      'mark.c3': 'End',
      'mark.c4': 'Finish'
    },

    /* ================= FRANÇAIS ================= */
    fr: {
      'html.lang': 'fr',
      'html.dir': 'ltr',
      'locale': 'fr-FR',
      'doc.title': 'Éclipse AR 2026 · 12 août',

      'app.kicker': 'Éclipse Solaire Totale',
      'app.title': '12 août 2026',
      'app.subtitle': 'La première éclipse totale visible depuis la péninsule Ibérique en plus d\'un siècle',
      'app.geo': 'Utiliser ma position GPS',
      'app.geoActive': 'Position GPS active',
      'app.geoLocating': 'Localisation…',
      'app.geoError': 'Impossible d\'obtenir la position : ',
      'app.geoHttps': 'Le GPS nécessite HTTPS.',
      'app.geoUnsupported': 'Votre navigateur ne prend pas en charge la géolocalisation.',
      'app.myLocation': 'Ma position',
      'app.mapPoint': 'Point de la carte',

      'card.phases': 'Phases de l\'éclipse à votre position',
      'card.sim': 'Simulation en temps réel',
      'card.sun': 'Où se trouve le Soleil en ce moment',
      'card.local': 'Circonstances locales',
      'card.band': 'Bande de totalité',
      'card.cities': 'Villes de référence',
      'card.safety': 'Sécurité et conseils',
      'card.tools': 'Outils',

      'ro.mag': 'Magnitude',
      'ro.obs': 'Soleil couvert',
      'ro.phase': 'Phase',
      'ro.total': 'TOTALE',
      'ro.partial': 'Partielle',
      'ro.before': 'Pas encore',
      'ro.after': 'Terminée',

      'scrub.explore': 'Explorer l\'éclipse',
      'scrub.live': 'EN DIRECT',
      'scrub.drag': 'Faites glisser pour avancer / reculer',
      'scrub.back': 'Revenir au direct',

      'sun.az': 'Azimut (depuis le Nord)',
      'sun.alt': 'Hauteur au-dessus de l\'horizon',
      'sun.dir': 'Orientation',
      'sun.set': 'Coucher du soleil aujourd\'hui',
      'sun.below': '(sous l\'horizon)',
      'sun.note': 'La boussole indique le <b>Nord en haut</b> ; le bord est l\'horizon et le centre, le zénith. La ligne orange est la trajectoire que suivra le Soleil pendant l\'éclipse, et le point blanc marque l\'instant du maximum.',

      'legend.center': 'Ligne centrale',
      'legend.band': 'Bande de totalité',
      'legend.you': 'Votre position',
      'map.note': 'La ligne centrale la plus proche est à <b>{km} km</b> vers le <b>{dir}</b> ({lat}°, {lon}°). Touchez la carte pour calculer n\'importe quel autre point.',
      'map.noteSimple': 'Touchez la carte pour calculer les circonstances en tout point.',
      'map.offline': 'Carte indisponible hors connexion.',

      'cities.note': 'En <b>orange</b>, celles situées dans la bande de totalité.',

      'phases.note': 'Heures dans votre fuseau horaire local. « Alt » est la hauteur du Soleil au-dessus de l\'horizon à cet instant.',
      'phases.noTotality': 'Pas de totalité à cet endroit : l\'éclipse sera partielle et vous ne devrez <b>jamais</b> retirer votre filtre.',

      'phase.c1': 'C1 · Premier contact',
      'phase.c1d': 'La Lune touche le bord du Soleil. L\'éclipse partielle commence. <b>Filtre en place.</b>',
      'phase.c2': 'C2 · Début de la totalité',
      'phase.c2d': 'Anneau de diamant et grains de Baily. <b>Vous pouvez retirer le filtre.</b>',
      'phase.max': 'Maximum de l\'éclipse',
      'phase.maxd': 'Le point culminant. Couronne solaire, planètes et étoiles visibles.',
      'phase.c3': 'C3 · Fin de la totalité',
      'phase.c3d': 'Le Soleil réapparaît. <b>Remettez le filtre MAINTENANT</b>, avant le premier éclat.',
      'phase.c4': 'C4 · Dernier contact',
      'phase.c4d': 'La Lune quitte le disque solaire. Fin de l\'éclipse.',

      'cd.c1': 'Avant le DÉBUT (C1)',
      'cd.c2': '⚡ Avant la TOTALITÉ (C2)',
      'cd.max': 'Avant le MAXIMUM',
      'cd.c3': '🔴 TOTALITÉ restante — regardez maintenant !',
      'cd.c4': 'Avant la FIN (C4)',
      'cd.done': '✨ L\'éclipse est terminée',
      'cd.notVisible': 'Éclipse non visible d\'ici',
      'cd.days': 'Jours', 'cd.hours': 'Heures', 'cd.min': 'Min', 'cd.sec': 'Sec',

      'alert.notVisible': 'Depuis <b>{place}</b> cette éclipse <b>n\'est pas visible</b>. L\'ombre de la Lune ne passe pas ici.',
      'alert.total': '<b>Vous êtes dans la bande de totalité !</b> Vous verrez la couronne solaire pendant <b>{dur}</b> à <b>{time}</b>.',
      'alert.partial': 'D\'ici l\'éclipse sera <b>partielle</b> ({pct} % du Soleil couvert). La ligne centrale de totalité passe à <b>{km} km</b> vers le <b>{dir}</b>. Cela vaut vraiment le déplacement : une éclipse partielle à 99 % n\'a <i>rien</i> à voir avec la totalité.',
      'alert.belowHorizon': 'Le maximum de l\'éclipse se produit <b>après le coucher du Soleil</b> à votre position. Vous devez vous déplacer vers l\'ouest ou le nord.',
      'alert.veryLow': 'Le Soleil ne sera qu\'à <b>{alt}°</b> de hauteur au maximum — à peine trois doigts à bout de bras. Il vous faut un horizon ouest <b>totalement dégagé</b> (côte, belvédère ou plaine).',
      'alert.low': 'Le Soleil sera bas, à <b>{alt}°</b> de hauteur au maximum. Cherchez un endroit avec l\'horizon ouest libre de montagnes et de bâtiments.',
      'alert.sunset': 'Le Soleil se couchera <b>avant la fin</b> de l\'éclipse : il disparaîtra encore mordu par la Lune. Un spectacle photographique saisissant.',

      'stat.type': 'Type d\'éclipse',
      'stat.totality': 'Durée de la totalité',
      'stat.mag': 'Magnitude maximale',
      'stat.obs': 'Soleil couvert (surface)',
      'stat.duration': 'Durée totale de l\'événement',
      'stat.altMax': 'Hauteur du Soleil au maximum',
      'stat.azMax': 'Azimut au maximum',
      'stat.ratio': 'Taille Lune / Soleil',
      'stat.noData': 'Aucune donnée.',
      'badge.total': 'Totale', 'badge.annular': 'Annulaire', 'badge.partial': 'Partielle',

      'safety.1': '<b>Ne regardez jamais le Soleil sans filtre homologué.</b> Il vous faut des lunettes d\'éclipse certifiées <b>ISO 12312-2</b>. Les lunettes de soleil, radiographies, CD ou verres fumés <b>ne protègent pas</b> et peuvent causer une cécité permanente.',
      'safety.2': '<b>Uniquement pendant la totalité</b> (entre C2 et C3) vous pouvez et devez regarder à l\'œil nu : c\'est le seul moment où la couronne est visible. Remettez le filtre <b>dès le premier éclat</b>.',
      'safety.3': 'Appareils photo, jumelles et télescopes ont besoin d\'un <b>filtre solaire devant l\'objectif</b>. Un filtre d\'oculaire peut éclater sous la chaleur.',
      'safety.4': 'Dans la péninsule Ibérique l\'éclipse se produit avec le Soleil <b>très bas</b> sur l\'horizon ouest. Cherchez un point de vue dégagé : une colline, la côte ou une plaine sans montagnes à l\'ouest. <b>Vérifiez votre horizon la veille à la même heure.</b>',

      'tools.ics': '📅 Ajouter au calendrier',
      'tools.notify': '🔔 Alertes sur le mobile',
      'tools.share': '🔗 Partager',
      'tools.install': '⬇️ Installer l\'app',
      'tools.notifyOn': '🔔 Alertes activées',
      'tools.notifyOnBody': 'Je vous préviendrai à chaque phase de l\'éclipse. Gardez l\'app ouverte ce jour-là.',
      'tools.notifyUnsupported': 'Votre navigateur ne prend pas en charge les notifications.',
      'tools.copied': 'Lien copié.',
      'tools.installHelp': 'Sur iPhone : appuyez sur « Partager » dans Safari et choisissez « Sur l\'écran d\'accueil ».\nSur Android : menu ⋮ → « Installer l\'application ».',

      'ics.c1': '🌒 Début de l\'éclipse solaire (C1)',
      'ics.c1d': 'Mettez vos lunettes d\'éclipse ISO 12312-2. {place}',
      'ics.c2': '🌑 TOTALITÉ de l\'éclipse',
      'ics.c2d': 'Durée : {dur}. Retirez le filtre SEULEMENT maintenant.',
      'ics.max': '☀️ Maximum de l\'éclipse',
      'ics.maxd': 'Magnitude {mag} · {pct} % couvert',
      'ics.c4': '🌘 Fin de l\'éclipse (C4)',
      'ics.c4d': 'Dernier contact.',
      'share.total': 'Éclipse totale le 12/08/2026 depuis {place} : totalité à {time} pendant {dur}.',
      'share.other': 'Éclipse solaire du 12/08/2026 depuis {place}.',

      'notif.c1': '🌒 L\'éclipse commence', 'notif.c1b': 'Premier contact. Mettez votre filtre solaire.',
      'notif.c2pre': '⚡ Totalité dans 1 minute', 'notif.c2preb': 'Préparez-vous. Anneau de diamant imminent.',
      'notif.c2': '🌑 TOTALITÉ !', 'notif.c2b': 'Retirez le filtre et admirez la couronne !',
      'notif.c3': '⚠️ Fin de la totalité', 'notif.c3b': 'REMETTEZ LE FILTRE MAINTENANT !',
      'notif.c4': '🌘 Fin de l\'éclipse', 'notif.c4b': 'Dernier contact. Rendez-vous le 2 août 2027.',

      'dock.ar': 'Mode Réalité Augmentée',
      'ar.track': 'Trajectoire',
      'ar.disk': 'Disque solaire',
      'ar.cal': '🎯 Calibrer sur le Soleil',
      'ar.hintDefault': 'Pointez le mobile là où la flèche l\'indique',
      'ar.starting': 'Démarrage de la caméra…',
      'ar.waitSensors': 'En attente des capteurs d\'orientation… bougez un peu le mobile.',
      'ar.moveCompass': 'Bougez le mobile pour activer la boussole.',
      'ar.relative': '⚠️ Votre mobile n\'a pas de <b>boussole absolue</b> : le cap peut être décalé. Appuyez sur <b>Calibrer</b> et marquez où est le Soleil.',
      'ar.totality': '🌑 <b>TOTALITÉ</b> — retirez le filtre et regardez directement',
      'ar.soon': '⚡ Totalité imminente — <b>préparez-vous</b>',
      'ar.partial': '⛔ Éclipse partielle en cours — <b>NE regardez PAS sans filtre ISO 12312-2</b>',
      'ar.belowHorizon': 'Le Soleil est sous l\'horizon. Le réticule indique où il sera.',
      'ar.normal': 'La ligne orange est la trajectoire du Soleil pendant l\'éclipse. ⛔ Ne regardez jamais le Soleil sans filtre.',
      'ar.calibHint': '🎯 Visez le <b>vrai Soleil</b> avec le réticule et touchez l\'écran. Si vous ne le voyez pas, utilisez un repère connu.',
      'ar.calibDone': '✅ Boussole corrigée de <b>{off}°</b>. Mémorisé pour la prochaine fois.',
      'ar.calibCancel': 'Calibrage annulé.',
      'ar.calibReset': 'Correction de boussole réinitialisée.',
      'ar.noCamera': '⚠️ Pas d\'accès à la caméra.<br>Seule la superposition est affichée.',
      'ar.noSensors': 'Pas d\'autorisation pour les capteurs d\'orientation. Sur iPhone, rechargez et acceptez l\'invite « Mouvement et orientation ».',
      'ar.sun': '☀ SOLEIL',
      'ar.sunBelow': '☀ SOLEIL (sous l\'horizon)',
      'ar.turnHere': 'Tournez par ici',
      'ar.covered': 'Couvert',
      'ar.in': 'dans',
      'ar.finished': 'Éclipse terminée',
      'ar.relTag': 'boussole relative',

      'footer': 'Calculs dérivés des <b>éléments besséliens de la NASA/GSFC</b> (F. Espenak), éphémérides VSOP87/ELP2000-85, ΔT = 71,4 s. Tout est calculé sur votre appareil, sans serveur.<br>Vérifié avec les données de l\'<b>IGN</b> (Oviedo : magnitude 1,015, totalité 1m 48s).<br><br>Fait avec soin pour ceux qui attendent ce jour depuis des décennies. ☀️🌑',

      'v.btnOff': '🔈 Activer la voix',
      'v.btnOn': '🔊 Voix activée',
      'v.btnBrief': '🗣️ Que vais-je voir ?',
      'v.ready': 'Voix activée. Je vous guiderai à chaque phase de l\u2019éclipse.',
      'v.noVoice': 'Votre appareil n\u2019a aucune voix installée pour cette langue.',
      'v.fallback': 'Votre téléphone n\u2019a pas de voix catalane. J\u2019utiliserai l\u2019espagnole.',
      'v.briefWhere': 'Vous êtes à {place}.',
      'v.briefTotal': 'D\u2019ici vous verrez une éclipse totale. La totalité commence à {c2} et dure {dur}.',
      'v.briefPartial': 'D\u2019ici l\u2019éclipse sera partielle : la Lune couvrira jusqu\u2019à {pct} pour cent du Soleil.',
      'v.briefTimes': 'Elle commence à {c1} et se termine à {c4}.',
      'v.briefTimesSet': 'Ça commence à {c1}. L’éclipse ne se termine qu’à {c4}, mais d’ici le Soleil se couche avant, à {set} : la dernière partie se déroule avec le Soleil déjà sous l’horizon.',
      'v.briefSun': 'Tu dois regarder en direction {dir}, et très bas : le Soleil sera à seulement {alt} degrés au-dessus de l’horizon, {hand}.',
      'v.briefFinger1': 'la largeur d’un doigt à bout de bras',
      'v.briefFingers': 'comme {n} doigts serrés à bout de bras',
      'v.briefFist': 'comme un poing fermé à bout de bras',
      'v.briefBlocked': 'Attention à une chose : depuis ce point précis, à {time} le Soleil passera derrière le relief et tu ne le reverras plus. Si tu veux l’éclipse entière, cherche un endroit plus dégagé de ce côté.',
      'v.briefClear': 'Bonne nouvelle : d’ici, aucune montagne ne te cache le Soleil.',
      'v.briefBlockedLate': 'Encore une chose : tu verras la totalité, mais à {time} le Soleil passera derrière le relief et tu rateras la fin de l’éclipse.',
      'v.briefCloudsBad': 'Pour l’instant la prévision n’aide pas : {low} pour cent de nuages bas à cette heure, et ce sont eux qui cachent le Soleil.',
      'v.briefCloudsSome': 'La prévision donne quelques nuages bas, {low} pour cent, à cette heure.',
      'v.briefCloudsGood': 'La prévision donne un ciel dégagé à cette heure.',
      'v.briefLow': 'Très bas. Il vous faut un horizon ouest totalement dégagé.',
      'v.briefFilter': 'Rappel : gardez votre filtre solaire en permanence, sauf pendant la totalité.',
      'v.briefFilterAlways': 'Rappel : il n\'y a pas de totalité ici. Vous ne devez retirer le filtre solaire à aucun moment.',
      'cd.tap': '🔊 Touchez l\u2019horloge et je vous le dirai à voix haute',
      'v.greet': 'Il reste {t} avant le début de l\u2019éclipse solaire totale.',
      'v.greetC2': 'L\u2019éclipse a déjà commencé. Il reste {t} avant la totalité.',
      'v.greetNow': 'Vous êtes en pleine totalité. Levez les yeux !',
      'v.greetPartial': 'L\u2019éclipse a déjà commencé. Filtre en place.',
      'v.greetOver': 'L\u2019éclipse du 12 août est déjà passée.',
      'v.greetNoEcl': 'D\u2019ici, l\u2019éclipse n\u2019est pas visible.',
      'tl.voice': 'Laisser parler l\u2019app',
      'tl.voiceOn': 'L\u2019app vous parle',
      'tl.voiceDesc': 'Vous guide à chaque phase, sans écran',
      'tl.brief': 'Résumé',
      'tl.briefDesc': 'Résumé parlé',
      'tl.ics': 'Calendrier',
      'tl.icsDesc': 'Avec rappels',
      'tl.notify': 'Alertes',
      'tl.notifyDesc': 'Notifications',
      'tl.share': 'Partager',
      'tl.shareDesc': 'Envoyer',
      'tl.install': 'Installer',
      'tl.installDesc': 'App sur le mobile',
      'v.h1': 'Une heure avant le début de l\u2019éclipse. Bon moment pour vous installer et vérifier que votre horizon ouest est dégagé.',
      'v.m10': 'Dix minutes avant le premier contact. Mettez vos lunettes d\u2019éclipse maintenant.',
      'v.c1': 'L\u2019éclipse a commencé. La Lune touche le bord du Soleil. Ne regardez pas sans filtre.',
      'v.p25': 'Un quart du Soleil est couvert. Regardez l\u2019ombre des arbres : chaque trou entre les feuilles projette au sol une image du Soleil en croissant.',
      'v.p50': 'La moitié du Soleil est couverte. La lumière change : les couleurs s\u2019aplatissent et les ombres deviennent plus nettes.',
      'v.p75': 'Trois quarts couverts. Vous allez sentir la température baisser et entendre les oiseaux se taire.',
      'v.m5': 'Cinq minutes avant la totalité. Regardez vers l\u2019ouest : vous verrez arriver l\u2019ombre de la Lune comme un mur sombre.',
      'v.m1': 'Une minute avant la totalité. Préparez-vous à retirer le filtre quand je vous le dirai.',
      'v.s20': 'Vingt secondes. Observez une surface claire au sol : vous verrez peut-être des bandes d\u2019ombre frémissantes.',
      'v.c2': 'Totalité ! Enlevez les lunettes. Vous pouvez regarder le Soleil directement.',
      'v.mid': 'Vous contemplez la couronne solaire. Cherchez Vénus, brillante, près du Soleil. Et regardez l\u2019horizon : vous avez un coucher de soleil à trois cent soixante degrés tout autour de vous.',
      'v.c3warn': 'Attention. Lunettes remises, maintenant.',
      'v.c3': 'La totalité est terminée. Gardez le filtre jusqu\u2019à la fin.',
      'v.c4': 'Fin de l\u2019éclipse. Rendez-vous le 2 août 2027.',
      'v.pMax': 'Maximum de l\u2019éclipse. {pct} pour cent du Soleil est couvert. Ne retirez le filtre à aucun moment.',
      'v.dirSpoken': ['nord','nord-nord-est','nord-est','est-nord-est','est','est-sud-est','sud-est','sud-sud-est','sud','sud-sud-ouest','sud-ouest','ouest-sud-ouest','ouest','ouest-nord-ouest','nord-ouest','nord-nord-ouest'],
      'dir': ['N','NNE','NE','ENE','E','ESE','SE','SSE','S','SSO','SO','OSO','O','ONO','NO','NNO'],
      'mark.totality': 'Total',
      'mark.c1': 'Début',
      'mark.c2': 'Total',
      'mark.max': 'Max',
      'mark.c3': 'Fin',
      'mark.c4': 'Final'
    },

    /* ================= DEUTSCH ================= */
    de: {
      'html.lang': 'de',
      'html.dir': 'ltr',
      'locale': 'de-DE',
      'doc.title': 'Eclipse AR 2026 · 12. August',

      'app.kicker': 'Totale Sonnenfinsternis',
      'app.title': '12. August 2026',
      'app.subtitle': 'Die erste totale Sonnenfinsternis auf der Iberischen Halbinsel seit über einem Jahrhundert',
      'app.geo': 'Meinen GPS-Standort verwenden',
      'app.geoActive': 'GPS-Standort aktiv',
      'app.geoLocating': 'Standort wird ermittelt…',
      'app.geoError': 'Standort konnte nicht ermittelt werden: ',
      'app.geoHttps': 'GPS erfordert HTTPS.',
      'app.geoUnsupported': 'Dein Browser unterstützt keine Standortbestimmung.',
      'app.myLocation': 'Mein Standort',
      'app.mapPoint': 'Kartenpunkt',

      'card.phases': 'Finsternisphasen an deinem Standort',
      'card.sim': 'Simulation in Echtzeit',
      'card.sun': 'Wo die Sonne gerade steht',
      'card.local': 'Örtliche Umstände',
      'card.band': 'Totalitätszone',
      'card.cities': 'Referenzstädte',
      'card.safety': 'Sicherheit und Tipps',
      'card.tools': 'Werkzeuge',

      'ro.mag': 'Größe',
      'ro.obs': 'Sonne bedeckt',
      'ro.phase': 'Phase',
      'ro.total': 'TOTAL',
      'ro.partial': 'Partiell',
      'ro.before': 'Noch nicht',
      'ro.after': 'Vorbei',

      'scrub.explore': 'Finsternis erkunden',
      'scrub.live': 'LIVE',
      'scrub.drag': 'Ziehen, um vor- oder zurückzuspulen',
      'scrub.back': 'Zurück zu live',

      'sun.az': 'Azimut (von Norden)',
      'sun.alt': 'Höhe über dem Horizont',
      'sun.dir': 'Richtung',
      'sun.set': 'Sonnenuntergang heute',
      'sun.below': '(unter dem Horizont)',
      'sun.note': 'Der Kompass zeigt <b>Norden oben</b>; der Rand ist der Horizont, die Mitte der Zenit. Die orange Linie ist die Bahn der Sonne während der Finsternis, der weiße Punkt markiert das Maximum.',

      'legend.center': 'Zentrallinie',
      'legend.band': 'Totalitätszone',
      'legend.you': 'Dein Standort',
      'map.note': 'Die nächste Zentrallinie liegt <b>{km} km</b> in Richtung <b>{dir}</b> ({lat}°, {lon}°). Tippe auf die Karte, um einen beliebigen anderen Punkt zu berechnen.',
      'map.noteSimple': 'Tippe auf die Karte, um die Umstände an jedem Punkt zu berechnen.',
      'map.offline': 'Karte offline nicht verfügbar.',

      'cities.note': 'In <b>Orange</b> die Städte innerhalb der Totalitätszone.',

      'phases.note': 'Zeiten in deiner lokalen Zeitzone. «Alt» ist die Höhe der Sonne über dem Horizont in diesem Moment.',
      'phases.noTotality': 'Keine Totalität an diesem Ort: Die Finsternis bleibt partiell und du darfst den Filter <b>niemals</b> abnehmen.',

      'phase.c1': 'C1 · Erster Kontakt',
      'phase.c1d': 'Der Mond berührt den Sonnenrand. Die partielle Phase beginnt. <b>Filter aufsetzen.</b>',
      'phase.c2': 'C2 · Beginn der Totalität',
      'phase.c2d': 'Diamantring und Baily\'sche Perlen. <b>Jetzt darfst du den Filter abnehmen.</b>',
      'phase.max': 'Maximum der Finsternis',
      'phase.maxd': 'Der Höhepunkt. Sonnenkorona, Planeten und Sterne sichtbar.',
      'phase.c3': 'C3 · Ende der Totalität',
      'phase.c3d': 'Die Sonne kommt zurück. <b>Filter SOFORT wieder auf</b>, vor dem ersten Aufblitzen.',
      'phase.c4': 'C4 · Letzter Kontakt',
      'phase.c4d': 'Der Mond verlässt die Sonnenscheibe. Ende der Finsternis.',

      'cd.c1': 'Bis zum BEGINN (C1)',
      'cd.c2': '⚡ Bis zur TOTALITÄT (C2)',
      'cd.max': 'Bis zum MAXIMUM',
      'cd.c3': '🔴 TOTALITÄT verbleibend — jetzt hinschauen!',
      'cd.c4': 'Bis zum ENDE (C4)',
      'cd.done': '✨ Die Finsternis ist vorbei',
      'cd.notVisible': 'Finsternis von hier nicht sichtbar',
      'cd.days': 'Tage', 'cd.hours': 'Std', 'cd.min': 'Min', 'cd.sec': 'Sek',

      'alert.notVisible': 'Von <b>{place}</b> aus ist diese Finsternis <b>nicht sichtbar</b>. Der Mondschatten zieht hier nicht vorbei.',
      'alert.total': '<b>Du bist in der Totalitätszone!</b> Du wirst die Sonnenkorona <b>{dur}</b> lang um <b>{time}</b> sehen.',
      'alert.partial': 'Von hier aus ist die Finsternis <b>partiell</b> ({pct} % der Sonne bedeckt). Die Zentrallinie der Totalität verläuft <b>{km} km</b> in Richtung <b>{dir}</b>. Die Fahrt lohnt sich enorm: 99 % partiell ist <i>nichts</i> im Vergleich zur Totalität.',
      'alert.belowHorizon': 'Das Maximum tritt an deinem Standort <b>nach Sonnenuntergang</b> ein. Du musst nach Westen oder Norden ausweichen.',
      'alert.veryLow': 'Die Sonne steht im Maximum nur <b>{alt}°</b> hoch — kaum drei Fingerbreit bei ausgestrecktem Arm. Du brauchst einen <b>völlig freien</b> Westhorizont (Küste, Aussichtspunkt oder Ebene).',
      'alert.low': 'Die Sonne steht tief, <b>{alt}°</b> hoch im Maximum. Suche einen Ort mit freiem Westhorizont ohne Berge und Gebäude.',
      'alert.sunset': 'Die Sonne geht <b>vor dem Ende</b> der Finsternis unter: Sie verschwindet noch angebissen vom Mond. Ein grandioses Fotomotiv.',

      'stat.type': 'Art der Finsternis',
      'stat.totality': 'Dauer der Totalität',
      'stat.mag': 'Maximale Größe',
      'stat.obs': 'Sonne bedeckt (Fläche)',
      'stat.duration': 'Gesamtdauer des Ereignisses',
      'stat.altMax': 'Sonnenhöhe im Maximum',
      'stat.azMax': 'Azimut im Maximum',
      'stat.ratio': 'Größe Mond / Sonne',
      'stat.noData': 'Keine Daten.',
      'badge.total': 'Total', 'badge.annular': 'Ringförmig', 'badge.partial': 'Partiell',

      'safety.1': '<b>Schau niemals ohne zertifizierten Filter in die Sonne.</b> Du brauchst eine <b>ISO 12312-2</b> zertifizierte Finsternisbrille. Sonnenbrillen, Röntgenbilder, CDs oder berußtes Glas <b>schützen nicht</b> und können zu dauerhafter Erblindung führen.',
      'safety.2': '<b>Nur während der Totalität</b> (zwischen C2 und C3) darfst und sollst du mit bloßem Auge hinsehen: Nur dann ist die Korona sichtbar. Setze den Filter <b>beim ersten Aufblitzen sofort</b> wieder auf.',
      'safety.3': 'Kameras, Ferngläser und Teleskope brauchen einen <b>Sonnenfilter vor dem Objektiv</b>. Ein Okularfilter kann durch die Hitze zerspringen.',
      'safety.4': 'Auf der Iberischen Halbinsel findet die Finsternis mit <b>sehr tief stehender</b> Sonne über dem Westhorizont statt. Suche einen freien Aussichtspunkt: einen Hügel, die Küste oder eine Ebene ohne Berge im Westen. <b>Prüfe deinen Horizont am Vortag zur selben Uhrzeit.</b>',

      'tools.ics': '📅 Zum Kalender hinzufügen',
      'tools.notify': '🔔 Handy-Benachrichtigungen',
      'tools.share': '🔗 Teilen',
      'tools.install': '⬇️ App installieren',
      'tools.notifyOn': '🔔 Benachrichtigungen aktiv',
      'tools.notifyOnBody': 'Ich melde mich bei jeder Phase der Finsternis. Lass die App an diesem Tag geöffnet.',
      'tools.notifyUnsupported': 'Dein Browser unterstützt keine Benachrichtigungen.',
      'tools.copied': 'Link kopiert.',
      'tools.installHelp': 'Auf dem iPhone: In Safari auf «Teilen» tippen und «Zum Home-Bildschirm» wählen.\nAuf Android: Menü ⋮ → «App installieren».',

      'ics.c1': '🌒 Sonnenfinsternis beginnt (C1)',
      'ics.c1d': 'Setze deine ISO 12312-2 Finsternisbrille auf. {place}',
      'ics.c2': '🌑 TOTALITÄT der Finsternis',
      'ics.c2d': 'Dauer: {dur}. Filter NUR jetzt abnehmen.',
      'ics.max': '☀️ Maximum der Finsternis',
      'ics.maxd': 'Größe {mag} · {pct} % bedeckt',
      'ics.c4': '🌘 Ende der Finsternis (C4)',
      'ics.c4d': 'Letzter Kontakt.',
      'share.total': 'Totale Sonnenfinsternis am 12.08.2026 von {place}: Totalität um {time} für {dur}.',
      'share.other': 'Sonnenfinsternis vom 12.08.2026 von {place}.',

      'notif.c1': '🌒 Finsternis beginnt', 'notif.c1b': 'Erster Kontakt. Setze den Sonnenfilter auf.',
      'notif.c2pre': '⚡ Totalität in 1 Minute', 'notif.c2preb': 'Mach dich bereit. Diamantring steht bevor.',
      'notif.c2': '🌑 TOTALITÄT!', 'notif.c2b': 'Filter abnehmen und die Korona ansehen!',
      'notif.c3': '⚠️ Totalität endet', 'notif.c3b': 'FILTER SOFORT WIEDER AUF!',
      'notif.c4': '🌘 Finsternis vorbei', 'notif.c4b': 'Letzter Kontakt. Bis zum 2. August 2027.',

      'dock.ar': 'Augmented-Reality-Modus',
      'ar.track': 'Bahn',
      'ar.disk': 'Sonnenscheibe',
      'ar.cal': '🎯 An der Sonne kalibrieren',
      'ar.hintDefault': 'Richte das Handy dorthin, wo der Pfeil zeigt',
      'ar.starting': 'Kamera wird gestartet…',
      'ar.waitSensors': 'Warte auf die Orientierungssensoren… bewege das Handy etwas.',
      'ar.moveCompass': 'Bewege das Handy, um den Kompass zu aktivieren.',
      'ar.relative': '⚠️ Dein Handy liefert keinen <b>absoluten Kompass</b>: Die Richtung kann verdreht sein. Tippe auf <b>Kalibrieren</b> und markiere, wo die Sonne steht.',
      'ar.totality': '🌑 <b>TOTALITÄT</b> — Filter abnehmen und direkt hinsehen',
      'ar.soon': '⚡ Totalität steht bevor — <b>bereitmachen</b>',
      'ar.partial': '⛔ Partielle Finsternis läuft — <b>NICHT ohne ISO 12312-2 Filter hinsehen</b>',
      'ar.belowHorizon': 'Die Sonne steht unter dem Horizont. Das Fadenkreuz zeigt, wo sie sein wird.',
      'ar.normal': 'Die orange Linie ist die Sonnenbahn während der Finsternis. ⛔ Niemals ohne Filter in die Sonne schauen.',
      'ar.calibHint': '🎯 Ziele mit dem Fadenkreuz auf die <b>echte Sonne</b> und tippe auf den Bildschirm. Wenn du sie nicht siehst, nimm einen bekannten Bezugspunkt.',
      'ar.calibDone': '✅ Kompass um <b>{off}°</b> korrigiert. Wird für das nächste Mal gemerkt.',
      'ar.calibCancel': 'Kalibrierung abgebrochen.',
      'ar.calibReset': 'Kompasskorrektur zurückgesetzt.',
      'ar.noCamera': '⚠️ Kein Kamerazugriff.<br>Es wird nur die Überlagerung angezeigt.',
      'ar.noSensors': 'Keine Berechtigung für die Orientierungssensoren. Lade auf dem iPhone neu und akzeptiere die Abfrage «Bewegung & Ausrichtung».',
      'ar.sun': '☀ SONNE',
      'ar.sunBelow': '☀ SONNE (unter dem Horizont)',
      'ar.turnHere': 'Hierhin drehen',
      'ar.covered': 'Bedeckt',
      'ar.in': 'in',
      'ar.finished': 'Finsternis vorbei',
      'ar.relTag': 'relativer Kompass',

      'footer': 'Berechnungen abgeleitet aus den <b>Besselschen Elementen der NASA/GSFC</b> (F. Espenak), Ephemeriden VSOP87/ELP2000-85, ΔT = 71,4 s. Alles wird auf deinem Gerät berechnet, ohne Server.<br>Geprüft gegen die Daten des <b>IGN</b> (Oviedo: Größe 1,015, Totalität 1m 48s).<br><br>Mit Sorgfalt gemacht für alle, die seit Jahrzehnten auf diesen Tag warten. ☀️🌑',

      'v.btnOff': '🔈 Stimme einschalten',
      'v.btnOn': '🔊 Stimme an',
      'v.btnBrief': '🗣️ Was werde ich sehen?',
      'v.ready': 'Stimme aktiviert. Ich führe dich durch jede Phase der Finsternis.',
      'v.noVoice': 'Auf deinem Gerät ist keine Stimme für diese Sprache installiert.',
      'v.fallback': 'Dein Handy hat keine katalanische Stimme. Ich nehme die spanische.',
      'v.briefWhere': 'Du bist in {place}.',
      'v.briefTotal': 'Von hier aus siehst du eine totale Finsternis. Die Totalität beginnt um {c2} und dauert {dur}.',
      'v.briefPartial': 'Von hier aus wird die Finsternis partiell: Der Mond bedeckt bis zu {pct} Prozent der Sonne.',
      'v.briefTimes': 'Sie beginnt um {c1} und endet um {c4}.',
      'v.briefTimesSet': 'Sie beginnt um {c1}. Die Finsternis endet erst um {c4}, aber von hier geht die Sonne schon um {set} unter: der letzte Teil läuft ab, wenn die Sonne bereits unter dem Horizont steht.',
      'v.briefSun': 'Du musst nach {dir} schauen, und sehr tief: die Sonne steht nur {alt} Grad über dem Horizont, {hand}.',
      'v.briefFinger1': 'etwa eine Fingerbreite bei ausgestrecktem Arm',
      'v.briefFingers': 'etwa {n} Finger nebeneinander bei ausgestrecktem Arm',
      'v.briefFist': 'etwa eine geschlossene Faust bei ausgestrecktem Arm',
      'v.briefBlocked': 'Eines noch: von genau diesem Punkt aus verschwindet die Sonne um {time} hinter dem Gelände und du siehst sie nicht wieder. Wenn du die ganze Finsternis willst, such dir dort drüben einen offeneren Platz.',
      'v.briefClear': 'Gute Nachricht: von hier verdeckt kein Berg die Sonne.',
      'v.briefBlockedLate': 'Noch etwas: die Totalität siehst du, aber um {time} verschwindet die Sonne hinter dem Gelände und der letzte Teil fällt aus.',
      'v.briefCloudsBad': 'Die Vorhersage hilft gerade nicht: {low} Prozent tiefe Wolken zu der Zeit, und die sind es, die die Sonne verdecken.',
      'v.briefCloudsSome': 'Die Vorhersage gibt etwas tiefe Bewölkung, {low} Prozent, zu der Zeit.',
      'v.briefCloudsGood': 'Die Vorhersage gibt klaren Himmel zu der Zeit.',
      'v.briefLow': 'Sehr tief. Du brauchst einen völlig freien Westhorizont.',
      'v.briefFilter': 'Denk daran: Sonnenfilter immer auf, außer während der Totalität.',
      'v.briefFilterAlways': 'Denk daran: Hier gibt es keine Totalität. Du darfst den Sonnenfilter zu keinem Zeitpunkt abnehmen.',
      'cd.tap': '🔊 Tippe auf die Uhr und ich sage es dir laut',
      'v.greet': 'Noch {t} bis zum Beginn der totalen Sonnenfinsternis.',
      'v.greetC2': 'Die Finsternis hat bereits begonnen. Noch {t} bis zur Totalität.',
      'v.greetNow': 'Du bist mitten in der Totalität. Schau nach oben!',
      'v.greetPartial': 'Die Finsternis hat bereits begonnen. Filter auf.',
      'v.greetOver': 'Die Finsternis vom 12. August ist bereits vorbei.',
      'v.greetNoEcl': 'Von hier aus ist die Finsternis nicht sichtbar.',
      'tl.voice': 'Die App sprechen lassen',
      'tl.voiceOn': 'Die App spricht',
      'tl.voiceDesc': 'Führt dich durch jede Phase, ohne Display',
      'tl.brief': 'Übersicht',
      'tl.briefDesc': 'Gesprochene Übersicht',
      'tl.ics': 'Kalender',
      'tl.icsDesc': 'Mit Erinnerungen',
      'tl.notify': 'Hinweise',
      'tl.notifyDesc': 'Benachrichtigungen',
      'tl.share': 'Teilen',
      'tl.shareDesc': 'Weitergeben',
      'tl.install': 'Installieren',
      'tl.installDesc': 'App aufs Handy',
      'v.h1': 'Noch eine Stunde bis zum Beginn der Finsternis. Guter Moment, dich einzurichten und zu prüfen, ob dein Westhorizont frei ist.',
      'v.m10': 'Zehn Minuten bis zum ersten Kontakt. Setz jetzt deine Finsternisbrille auf.',
      'v.c1': 'Die Finsternis hat begonnen. Der Mond berührt den Sonnenrand. Schau nicht ohne Filter hin.',
      'v.p25': 'Ein Viertel der Sonne ist bedeckt. Schau auf den Schatten der Bäume: Jede Lücke zwischen den Blättern wirft ein sichelförmiges Bild der Sonne auf den Boden.',
      'v.p50': 'Die Hälfte der Sonne ist bedeckt. Das Licht verändert sich: Die Farben werden flacher und die Schatten schärfer.',
      'v.p75': 'Drei Viertel bedeckt. Du wirst merken, wie die Temperatur fällt und die Vögel verstummen.',
      'v.m5': 'Fünf Minuten bis zur Totalität. Schau nach Westen: Du siehst den Mondschatten wie eine dunkle Wand herankommen.',
      'v.m1': 'Eine Minute bis zur Totalität. Mach dich bereit, den Filter abzunehmen, wenn ich es sage.',
      'v.s20': 'Zwanzig Sekunden. Beobachte eine helle Fläche am Boden: Vielleicht siehst du flirrende Schattenbänder.',
      'v.c2': 'Totalität! Brille ab. Du darfst jetzt direkt in die Sonne schauen.',
      'v.mid': 'Du siehst gerade die Sonnenkorona. Such die Venus, hell, nahe der Sonne. Und schau zum Horizont: Du hast einen Sonnenuntergang von dreihundertsechzig Grad rings um dich.',
      'v.c3warn': 'Achtung. Brille wieder auf, jetzt.',
      'v.c3': 'Die Totalität ist vorbei. Filter auf bis zum Schluss.',
      'v.c4': 'Ende der Finsternis. Bis zum 2. August 2027.',
      'v.pMax': 'Maximum der Finsternis. {pct} Prozent der Sonne sind bedeckt. Nimm den Filter zu keinem Zeitpunkt ab.',
      'v.dirSpoken': ['Norden','Nordnordost','Nordost','Ostnordost','Osten','Ostsüdost','Südost','Südsüdost','Süden','Südsüdwest','Südwest','Westsüdwest','Westen','Westnordwest','Nordwest','Nordnordwest'],
      'dir': ['N','NNO','NO','ONO','O','OSO','SO','SSO','S','SSW','SW','WSW','W','WNW','NW','NNW'],
      'mark.totality': 'Total',
      'mark.c1': 'Beginn',
      'mark.c2': 'Total',
      'mark.max': 'Max',
      'mark.c3': 'Ende',
      'mark.c4': 'Schluss'
    },

    /* ================= 中文 (CHINÈS) ================= */
    zh: {
      'html.lang': 'zh',
      'html.dir': 'ltr',
      'locale': 'zh-CN',
      'doc.title': '2026年日全食 AR · 8月12日',
      'app.kicker': '日全食',
      'app.title': '2026年8月12日',
      'app.subtitle': '一个多世纪以来伊比利亚半岛首次可见的日全食',
      'app.geo': '使用我的GPS定位',
      'app.geoActive': 'GPS定位已启用',
      'app.geoLocating': '定位中…',
      'app.geoError': '无法获取定位：',
      'app.geoHttps': '请注意，GPS需要HTTPS。',
      'app.geoUnsupported': '你的浏览器不支持定位功能。',
      'app.myLocation': '我的位置',
      'app.mapPoint': '地图上的点',
      'card.phases': '你所在位置的日食各阶段',
      'card.sim': '实时模拟',
      'card.sun': '太阳现在的位置',
      'card.local': '当地情况',
      'card.band': '全食带',
      'card.cities': '参考城市',
      'card.safety': '安全与建议',
      'card.tools': '工具',
      'ro.mag': '食分',
      'ro.obs': '太阳被遮挡',
      'ro.phase': '阶段',
      'ro.total': '全食',
      'ro.partial': '偏食',
      'ro.before': '尚未开始',
      'ro.after': '已结束',
      'scrub.explore': '探索这次日食',
      'scrub.live': '实时',
      'scrub.drag': '拖动以快进／倒退',
      'scrub.back': '回到实时',
      'sun.az': '方位角（从正北起算）',
      'sun.alt': '地平线以上高度',
      'sun.dir': '方向',
      'sun.set': '今天的日落',
      'sun.below': '（在地平线以下）',
      'sun.note': '罗盘<b>上方为北</b>；边缘是地平线，中心是天顶。橙色线是日食期间太阳走过的路径，白点标出食甚的时刻。',
      'legend.center': '中心线',
      'legend.band': '全食带',
      'legend.you': '你的位置',
      'map.note': '最近的中心线在<b>{dir}</b>方向<b>{km} 公里</b>处（{lat}°, {lon}°）。点击地图可计算任意其他地点。',
      'map.noteSimple': '点击地图即可计算任意地点的情况。',
      'map.offline': '离线时无法显示地图。',
      'cities.note': '<b>橙色</b>表示位于全食带之内。',
      'phases.note': '时间为你所在时区的当地时间。「高度」是那一刻太阳在地平线以上的高度。',
      'phases.noTotality': '此地没有全食阶段：只能看到偏食，<b>任何时候都不能</b>摘下滤光镜。',
      'phase.c1': 'C1 · 初亏',
      'phase.c1d': '月亮接触太阳边缘，偏食开始。<b>请戴上滤光镜。</b>',
      'phase.c2': 'C2 · 食既（全食开始）',
      'phase.c2d': '钻石环与倍利珠。<b>现在可以摘下滤光镜。</b>',
      'phase.max': '食甚',
      'phase.maxd': '最精彩的时刻。可见日冕、行星和星星。',
      'phase.c3': 'C3 · 生光（全食结束）',
      'phase.c3d': '太阳重新出现。<b>立刻戴回滤光镜</b>，在第一道亮光之前。',
      'phase.c4': 'C4 · 复圆',
      'phase.c4d': '月亮离开日面，日食结束。',
      'cd.c1': '距离开始（C1）还有',
      'cd.c2': '⚡ 距离全食（C2）还有',
      'cd.max': '距离食甚还有',
      'cd.c3': '🔴 全食剩余时间 — 快看！',
      'cd.c4': '距离结束（C4）还有',
      'cd.done': '✨ 日食已经结束',
      'cd.notVisible': '此地看不到这次日食',
      'cd.days': '天',
      'cd.hours': '小时',
      'cd.min': '分',
      'cd.sec': '秒',
      'alert.notVisible': '在<b>{place}</b>看<b>不到</b>这次日食，月影不经过这里。',
      'alert.total': '<b>你就在全食带里！</b>{time}起，你将看到<b>{dur}</b>的日冕。',
      'alert.partial': '此地只能看到<b>偏食</b>（太阳被遮挡 {pct} %）。全食中心线在<b>{dir}</b>方向<b>{km} 公里</b>处。非常值得跑一趟：99 %的偏食和全食<i>完全</i>不是一回事。',
      'alert.belowHorizon': '食甚发生时，你所在位置<b>太阳已经落下</b>。需要往西或往北移动。',
      'alert.veryLow': '食甚时太阳只有<b>{alt}°</b>高——比伸直手臂时三根手指的宽度多不了多少。你需要一个<b>完全开阔</b>的西方地平线（海边、观景台或平原）。',
      'alert.low': '食甚时太阳很低，只有<b>{alt}°</b>。请找一个西边没有山和建筑物遮挡的地方。',
      'alert.sunset': '太阳会在日食结束<b>之前</b>落下：落下时仍被月亮咬着一口。拍照效果极佳。',
      'stat.type': '日食类型',
      'stat.totality': '全食持续时间',
      'stat.mag': '最大食分',
      'stat.obs': '太阳被遮挡（面积）',
      'stat.duration': '全程持续时间',
      'stat.altMax': '食甚时太阳高度',
      'stat.azMax': '食甚时方位角',
      'stat.ratio': '月亮／太阳视直径比',
      'stat.noData': '没有数据。',
      'badge.total': '全食',
      'badge.annular': '环食',
      'badge.partial': '偏食',
      'safety.1': '<b>绝对不要在没有合格滤光镜的情况下直视太阳。</b>你需要通过<b>ISO 12312-2</b>认证的观测眼镜。太阳镜、X光片、光盘或熏黑的玻璃<b>都不管用</b>，可能造成永久失明。',
      'safety.2': '<b>只有在全食期间</b>（C2到C3之间）才可以、也应该用肉眼直接观看：这是唯一能看到日冕的时刻。<b>第一道亮光一出现就要立刻</b>戴回滤光镜。',
      'safety.3': '相机、双筒望远镜和天文望远镜都需要<b>在镜头前加太阳滤镜</b>。装在目镜上的滤镜可能被高温炸裂。',
      'safety.4': '在伊比利亚半岛，这次日食发生时太阳在西方地平线上<b>非常低</b>。请找一个开阔的观景点：山丘、海岸，或西边没有山的平原。<b>请在前一天的同一时刻先去看看地平线。</b>',
      'tools.ics': '📅 加入日历',
      'tools.notify': '🔔 手机提醒',
      'tools.share': '🔗 分享',
      'tools.install': '⬇️ 安装应用',
      'tools.notifyOn': '🔔 提醒已开启',
      'tools.notifyOnBody': '每个阶段我都会提醒你。当天请让应用保持打开。',
      'tools.notifyUnsupported': '你的浏览器不支持通知。',
      'tools.copied': '链接已复制。',
      'tools.installHelp': 'iPhone：点Safari的「分享」按钮，选择「添加到主屏幕」。\\nAndroid：菜单 ⋮ →「安装应用」。',
      'ics.c1': '🌒 日食开始（C1）',
      'ics.c1d': '戴上ISO 12312-2观测眼镜。{place}',
      'ics.c2': '🌑 日全食',
      'ics.c2d': '持续 {dur}。只有现在才能摘下滤光镜。',
      'ics.max': '☀️ 食甚',
      'ics.maxd': '食分 {mag} · 遮挡 {pct} %',
      'ics.c4': '🌘 日食结束（C4）',
      'ics.c4d': '复圆。',
      'share.total': '2026年8月12日日全食，在{place}：{time} 全食开始，持续 {dur}。',
      'share.other': '2026年8月12日日食，在{place}。',
      'notif.c1': '🌒 日食开始',
      'notif.c1b': '初亏。请戴上太阳滤光镜。',
      'notif.c2pre': '⚡ 1分钟后全食',
      'notif.c2preb': '准备好，钻石环即将出现。',
      'notif.c2': '🌑 全食！',
      'notif.c2b': '摘下滤光镜，看日冕！',
      'notif.c3': '⚠️ 全食结束',
      'notif.c3b': '立刻戴回滤光镜！',
      'notif.c4': '🌘 日食结束',
      'notif.c4b': '复圆。下一次是2027年8月2日。',
      'dock.ar': '增强现实模式',
      'ar.track': '轨迹',
      'ar.disk': '日面',
      'ar.cal': '🎯 对准太阳校准',
      'ar.hintDefault': '把手机朝箭头指的方向',
      'ar.starting': '正在启动相机…',
      'ar.waitSensors': '正在等待方向传感器…请轻轻晃动手机。',
      'ar.moveCompass': '晃动手机以启动罗盘。',
      'ar.relative': '⚠️ 你的手机没有<b>绝对罗盘</b>：方位可能有偏差。请按<b>校准</b>并指出太阳的位置。',
      'ar.totality': '🌑 <b>全食</b> — 摘下滤光镜，直接看',
      'ar.soon': '⚡ 全食即将开始 — <b>做好准备</b>',
      'ar.partial': '⛔ 偏食进行中 — <b>没有ISO 12312-2滤光镜请勿直视</b>',
      'ar.belowHorizon': '太阳在地平线以下。准星标出它将出现的位置。',
      'ar.normal': '橙色线是日食期间太阳的路径。⛔ 没有滤光镜请勿直视太阳。',
      'ar.calibHint': '🎯 把准星对准<b>真正的太阳</b>并点击屏幕。如果看不见太阳，可以用一个已知的参照物。',
      'ar.calibDone': '✅ 罗盘已修正 <b>{off}°</b>，下次会记住。',
      'ar.calibCancel': '已取消校准。',
      'ar.calibReset': '罗盘修正已重置。',
      'ar.noCamera': '⚠️ 无法使用相机。<br>只显示叠加图层。',
      'ar.noSensors': '没有方向传感器权限。iPhone请重新加载页面并允许「运动与方向」。',
      'ar.sun': '☀ 太阳',
      'ar.sunBelow': '☀ 太阳（在地平线以下）',
      'ar.turnHere': '转向这边',
      'ar.covered': '已遮挡',
      'ar.in': '还有',
      'ar.finished': '日食已结束',
      'ar.relTag': '相对罗盘',
      'footer': '计算基于<b>NASA/GSFC的贝塞尔要素</b>（F. Espenak）、VSOP87/ELP2000-85历表，ΔT = 71.4 秒。全部在你的设备上计算，不经过服务器。<br>已与西班牙<b>IGN</b>的数据核对（奥维耶多：食分 1.015，全食 1分48秒）。<br><br>献给等了几十年的人们。☀️🌑',
      'v.btnOff': '🔈 打开语音',
      'v.btnOn': '🔊 语音已打开',
      'v.btnBrief': '🗣️ 我会看到什么？',
      'v.ready': '语音已打开。日食的每个阶段我都会提醒你。',
      'v.noVoice': '你的设备没有安装这个语言的语音。',
      'v.fallback': '你的手机没有中文语音，将使用其他语言的语音。',
      'v.briefWhere': '你在{place}。',
      'v.briefTotal': '在这里你会看到日全食。全食从{c2}开始，持续{dur}。',
      'v.briefPartial': '在这里只能看到偏食：月亮最多会遮住太阳的百分之{pct}。',
      'v.briefTimes': '{c1}开始，{c4}结束。',
      'v.briefTimesSet': '{c1}开始。日食本身要到{c4}才结束，但太阳在这里{set}就落下了：最后一段是在太阳落到地平线以下之后发生的。',
      'v.briefSun': '你要朝{dir}方向看，而且很低：太阳只在地平线以上{alt}度，{hand}。',
      'v.briefFinger1': '大约是伸直手臂时一根手指的宽度',
      'v.briefFingers': '大约是伸直手臂时{n}根手指并起来的宽度',
      'v.briefFist': '大约是伸直手臂时一个拳头的宽度',
      'v.briefBlocked': '有一点要注意：就在这个位置，{time}太阳就会落到地形后面，之后你再也看不到它。想看完整场日食，请往那个方向找一个更开阔的地方。',
      'v.briefClear': '好消息：从这里看过去，没有任何山挡住太阳。',
      'v.briefBlockedLate': '还有一点：全食你能看到，不过{time}太阳会落到地形后面，日食的最后一段你就看不到了。',
      'v.briefCloudsBad': '目前的预报不太理想：那个时段有百分之{low}的低云，挡住太阳的正是低云。',
      'v.briefCloudsSome': '预报显示那个时段有一些低云，百分之{low}。',
      'v.briefCloudsGood': '预报显示那个时段天空晴朗。',
      'v.briefLow': '非常低。你需要一个完全开阔的西方地平线。',
      'v.briefFilter': '记住：滤光镜要一直戴着，只有全食期间可以摘下。',
      'v.briefFilterAlways': '记住：这里没有全食，滤光镜任何时候都不能摘。',
      'cd.tap': '🔊 点一下时钟，我念给你听',
      'v.greet': '距离日全食开始还有{t}。',
      'v.greetC2': '日食已经开始。距离全食还有{t}。',
      'v.greetNow': '现在正是全食！快抬头看！',
      'v.greetPartial': '日食已经开始。请戴好滤光镜。',
      'v.greetOver': '8月12日的日食已经过去了。',
      'v.greetNoEcl': '这里看不到这次日食。',
      'tl.voice': '让应用说话',
      'tl.voiceOn': '应用会说话',
      'tl.voiceDesc': '不用看手机也能收到每个阶段的提醒',
      'tl.brief': '我会看到什么？',
      'tl.briefDesc': '语音摘要',
      'tl.ics': '日历',
      'tl.icsDesc': '含提醒',
      'tl.notify': '提醒',
      'tl.notifyDesc': '通知',
      'tl.share': '分享',
      'tl.shareDesc': '发送出去',
      'tl.install': '安装',
      'tl.installDesc': '装到手机上',
      'v.h1': '距离日食开始还有一小时。正好可以就位，并检查西边的地平线是否开阔。',
      'v.m10': '还有十分钟就开始了。现在请戴上观测眼镜。',
      'v.c1': '日食开始了。月亮已经碰到太阳的边缘。没有滤光镜请勿直视。',
      'v.p25': '太阳被遮住四分之一。看看树影：叶子之间的每个缝隙都会在地上投出一个月牙形的太阳。',
      'v.p50': '太阳被遮住一半。光线开始变化：颜色变得更平淡，影子更锐利。',
      'v.p75': '被遮住四分之三。你会感到气温下降，鸟也安静下来。',
      'v.m5': '距离全食还有五分钟。往西看：你会看到月影像一堵黑墙一样压过来。',
      'v.m1': '距离全食还有一分钟。准备好，我提醒你的时候就摘下滤光镜。',
      'v.s20': '二十秒。看看地上浅色的表面：也许能看到晃动的影带。',
      'v.c2': '全食！摘下眼镜，现在可以直接看太阳了。',
      'v.mid': '你正在看日冕。找一找金星，很亮，就在太阳附近。再看看地平线：四周是三百六十度的晚霞。',
      'v.c3warn': '注意。立刻戴上眼镜。',
      'v.c3': '全食结束了。滤光镜要戴到最后。',
      'v.c4': '日食结束。下一次是2027年8月2日。',
      'v.pMax': '食甚。太阳被遮住了百分之{pct}。任何时候都不要摘下滤光镜。',
      'mark.totality': '全食',
      'mark.c1': '开始',
      'mark.c2': '全食',
      'mark.max': '食甚',
      'mark.c3': '结束',
      'mark.c4': '复圆',
      'v.dirSpoken': ['正北','东北偏北','东北','东北偏东','正东','东南偏东','东南','东南偏南','正南','西南偏南','西南','西南偏西','正西','西北偏西','西北','西北偏北'],
      'dir': ['北','东北偏北','东北','东北偏东','东','东南偏东','东南','东南偏南','南','西南偏南','西南','西南偏西','西','西北偏西','西北','西北偏北'],
    },

    /* ================= العربية (ÀRAB) ================= */
    ar: {
      'html.lang': 'ar',
      'html.dir': 'rtl',
      /* Cifras latinas (la mayoría de arabófonos aquí son magrebíes y las usan)
         y reloj de 24 h: «08:29 م» para un eclipse se presta a leerlo como
         la mañana, y aquí una hora mal leída es perderse la totalidad. */
      'locale': 'ar-u-nu-latn-hc-h23',
      'doc.title': 'كسوف 2026 بالواقع المعزز · 12 أغسطس',
      'app.kicker': 'كسوف كلي للشمس',
      'app.title': '12 أغسطس 2026',
      'app.subtitle': 'أول كسوف كلي يُرى من شبه الجزيرة الأيبيرية منذ أكثر من قرن',
      'app.geo': 'استخدام موقعي عبر GPS',
      'app.geoActive': 'تحديد الموقع مُفعَّل',
      'app.geoLocating': 'جارٍ تحديد الموقع…',
      'app.geoError': 'تعذَّر تحديد الموقع: ',
      'app.geoHttps': 'تذكَّر أن GPS يحتاج إلى HTTPS.',
      'app.geoUnsupported': 'متصفحك لا يدعم تحديد الموقع.',
      'app.myLocation': 'موقعي',
      'app.mapPoint': 'نقطة على الخريطة',
      'card.phases': 'مراحل الكسوف في موقعك',
      'card.sim': 'محاكاة في الزمن الحقيقي',
      'card.sun': 'أين الشمس الآن',
      'card.local': 'الظروف المحلية',
      'card.band': 'نطاق الكلية',
      'card.cities': 'مدن مرجعية',
      'card.safety': 'السلامة ونصائح',
      'card.tools': 'أدوات',
      'ro.mag': 'القدر',
      'ro.obs': 'الجزء المحجوب',
      'ro.phase': 'المرحلة',
      'ro.total': 'كلي',
      'ro.partial': 'جزئي',
      'ro.before': 'لم يبدأ بعد',
      'ro.after': 'انتهى',
      'scrub.explore': 'استكشف الكسوف',
      'scrub.live': 'مباشر',
      'scrub.drag': 'اسحب للتقديم أو الإرجاع',
      'scrub.back': 'العودة إلى المباشر',
      'sun.az': 'السمت (من الشمال)',
      'sun.alt': 'الارتفاع فوق الأفق',
      'sun.dir': 'الاتجاه',
      'sun.set': 'غروب الشمس اليوم',
      'sun.below': '(تحت الأفق)',
      'sun.note': 'تُظهر البوصلة <b>الشمال في الأعلى</b>؛ الحافة هي الأفق والمركز هو السمت. الخط البرتقالي هو مسار الشمس أثناء الكسوف، والنقطة البيضاء تحدد لحظة الذروة.',
      'legend.center': 'الخط المركزي',
      'legend.band': 'نطاق الكلية',
      'legend.you': 'موقعك',
      'map.note': 'أقرب خط مركزي يبعد <b>{km} كم</b> باتجاه <b>{dir}</b> ({lat}°، {lon}°). المس الخريطة لحساب أي نقطة أخرى.',
      'map.noteSimple': 'المس الخريطة لحساب الظروف في أي نقطة.',
      'map.offline': 'الخريطة غير متاحة دون اتصال.',
      'cities.note': 'باللون <b>البرتقالي</b> المدن الواقعة داخل نطاق الكلية.',
      'phases.note': 'الأوقات بتوقيتك المحلي. «الارتفاع» هو ارتفاع الشمس فوق الأفق في تلك اللحظة.',
      'phases.noTotality': 'لا توجد مرحلة كلية هنا: سيكون الكسوف جزئياً و<b>لا يجوز</b> نزع المرشِّح في أي لحظة.',
      'phase.c1': 'C1 · التماس الأول',
      'phase.c1d': 'يلامس القمر حافة الشمس. يبدأ الكسوف الجزئي. <b>ضع المرشِّح.</b>',
      'phase.c2': 'C2 · بداية الكلية',
      'phase.c2d': 'حلقة الماس وخرزات بيلي. <b>يمكنك الآن نزع المرشِّح.</b>',
      'phase.max': 'ذروة الكسوف',
      'phase.maxd': 'اللحظة الأروع. الإكليل الشمسي والكواكب والنجوم تُرى بالعين.',
      'phase.c3': 'C3 · نهاية الكلية',
      'phase.c3d': 'تعود الشمس للظهور. <b>ضع المرشِّح فوراً</b>، قبل أول وميض.',
      'phase.c4': 'C4 · التماس الأخير',
      'phase.c4d': 'يغادر القمر قرص الشمس. انتهى الكسوف.',
      'cd.c1': 'يتبقى على البداية (C1)',
      'cd.c2': '⚡ يتبقى على الكلية (C2)',
      'cd.max': 'يتبقى على الذروة',
      'cd.c3': '🔴 يتبقى من الكلية — انظر الآن!',
      'cd.c4': 'يتبقى على النهاية (C4)',
      'cd.done': '✨ انتهى الكسوف',
      'cd.notVisible': 'الكسوف غير مرئي من هنا',
      'cd.days': 'يوم',
      'cd.hours': 'ساعة',
      'cd.min': 'دقيقة',
      'cd.sec': 'ثانية',
      'alert.notVisible': 'من <b>{place}</b> هذا الكسوف <b>غير مرئي</b>. ظل القمر لا يمر من هنا.',
      'alert.total': '<b>أنت داخل نطاق الكلية!</b> سترى الإكليل الشمسي مدة <b>{dur}</b> عند الساعة <b>{time}</b>.',
      'alert.partial': 'من هنا سيكون الكسوف <b>جزئياً</b> (محجوب {pct} % من الشمس). يمر الخط المركزي للكلية على بُعد <b>{km} كم</b> باتجاه <b>{dir}</b>. يستحق الأمر أن تتنقل: كسوف جزئي بنسبة 99 % <i>لا</i> يشبه الكلية إطلاقاً.',
      'alert.belowHorizon': 'تقع ذروة الكسوف <b>بعد غروب الشمس</b> في موقعك. عليك التوجه غرباً أو شمالاً.',
      'alert.veryLow': 'لن يتجاوز ارتفاع الشمس <b>{alt}°</b> عند الذروة — أي بعرض ثلاثة أصابع تقريباً والذراع ممدودة. تحتاج إلى أفق غربي <b>خالٍ تماماً</b> (ساحل أو مطل أو سهل).',
      'alert.low': 'ستكون الشمس منخفضة، على ارتفاع <b>{alt}°</b> عند الذروة. ابحث عن مكان أفقه الغربي خالٍ من الجبال والمباني.',
      'alert.sunset': 'ستغرب الشمس <b>قبل انتهاء</b> الكسوف: ستغيب والقمر ما زال يقضم منها. مشهد تصويري مذهل.',
      'stat.type': 'نوع الكسوف',
      'stat.totality': 'مدة الكلية',
      'stat.mag': 'أقصى قدر',
      'stat.obs': 'المساحة المحجوبة',
      'stat.duration': 'المدة الكاملة للظاهرة',
      'stat.altMax': 'ارتفاع الشمس عند الذروة',
      'stat.azMax': 'السمت عند الذروة',
      'stat.ratio': 'نسبة حجم القمر إلى الشمس',
      'stat.noData': 'لا توجد بيانات.',
      'badge.total': 'كلي',
      'badge.annular': 'حلقي',
      'badge.partial': 'جزئي',
      'safety.1': '<b>لا تنظر إلى الشمس أبداً دون مرشِّح معتمد.</b> تحتاج نظارات كسوف معتمدة <b>ISO 12312-2</b>. النظارات الشمسية والأشعة السينية والأقراص المدمجة والزجاج المدخَّن <b>لا تنفع</b> وقد تسبب عمى دائماً.',
      'safety.2': '<b>أثناء الكلية فقط</b> (بين C2 وC3) يمكنك بل ينبغي أن تنظر بالعين المجردة: تلك هي اللحظة الوحيدة التي يُرى فيها الإكليل. أعد المرشِّح <b>فور ظهور أول وميض</b>.',
      'safety.3': 'الكاميرات والمناظير والتلسكوبات تحتاج <b>مرشِّحاً شمسياً أمام العدسة</b>. المرشِّح الموضوع على العينية قد ينفجر من الحرارة.',
      'safety.4': 'في شبه الجزيرة يقع الكسوف والشمس <b>منخفضة جداً</b> فوق الأفق الغربي. ابحث عن مطل مكشوف: تلة أو ساحل أو سهل بلا جبال غرباً. <b>تحقق من الأفق في اليوم السابق وفي الساعة نفسها.</b>',
      'tools.ics': '📅 إضافة إلى التقويم',
      'tools.notify': '🔔 تنبيهات على الهاتف',
      'tools.share': '🔗 مشاركة',
      'tools.install': '⬇️ تثبيت التطبيق',
      'tools.notifyOn': '🔔 التنبيهات مُفعَّلة',
      'tools.notifyOnBody': 'سأنبهك في كل مرحلة من الكسوف. أبقِ التطبيق مفتوحاً ذلك اليوم.',
      'tools.notifyUnsupported': 'متصفحك لا يدعم الإشعارات.',
      'tools.copied': 'تم نسخ الرابط.',
      'tools.installHelp': 'على iPhone: اضغط زر «مشاركة» في Safari واختر «إضافة إلى الشاشة الرئيسية».\\nعلى Android: القائمة ⋮ ← «تثبيت التطبيق».',
      'ics.c1': '🌒 يبدأ كسوف الشمس (C1)',
      'ics.c1d': 'ضع نظارات الكسوف ISO 12312-2. {place}',
      'ics.c2': '🌑 كلية الكسوف',
      'ics.c2d': 'المدة: {dur}. انزع المرشِّح الآن فقط.',
      'ics.max': '☀️ ذروة الكسوف',
      'ics.maxd': 'القدر {mag} · محجوب {pct} %',
      'ics.c4': '🌘 نهاية الكسوف (C4)',
      'ics.c4d': 'التماس الأخير.',
      'share.total': 'كسوف كلي في 12/08/2026 من {place}: الكلية عند {time} لمدة {dur}.',
      'share.other': 'كسوف الشمس في 12/08/2026 من {place}.',
      'notif.c1': '🌒 يبدأ الكسوف',
      'notif.c1b': 'التماس الأول. ضع المرشِّح الشمسي.',
      'notif.c2pre': '⚡ الكلية بعد دقيقة',
      'notif.c2preb': 'استعد. حلقة الماس على وشك الظهور.',
      'notif.c2': '🌑 الكلية!',
      'notif.c2b': 'انزع المرشِّح وانظر إلى الإكليل!',
      'notif.c3': '⚠️ نهاية الكلية',
      'notif.c3b': 'ضع المرشِّح فوراً!',
      'notif.c4': '🌘 نهاية الكسوف',
      'notif.c4b': 'التماس الأخير. إلى 2 أغسطس 2027.',
      'dock.ar': 'وضع الواقع المعزز',
      'ar.track': 'المسار',
      'ar.disk': 'قرص الشمس',
      'ar.cal': '🎯 معايرة بالشمس',
      'ar.hintDefault': 'وجّه الهاتف نحو ما يشير إليه السهم',
      'ar.starting': 'جارٍ تشغيل الكاميرا…',
      'ar.waitSensors': 'في انتظار مستشعرات الاتجاه… حرّك الهاتف قليلاً.',
      'ar.moveCompass': 'حرّك الهاتف لتفعيل البوصلة.',
      'ar.relative': '⚠️ هاتفك لا يوفر <b>بوصلة مطلقة</b>: قد يكون الاتجاه مُزاحاً. اضغط <b>معايرة</b> وحدد مكان الشمس.',
      'ar.totality': '🌑 <b>الكلية</b> — انزع المرشِّح وانظر مباشرة',
      'ar.soon': '⚡ الكلية وشيكة — <b>استعد</b>',
      'ar.partial': '⛔ كسوف جزئي جارٍ — <b>لا تنظر دون مرشِّح ISO 12312-2</b>',
      'ar.belowHorizon': 'الشمس تحت الأفق. تشير الشبكة إلى المكان الذي ستظهر فيه.',
      'ar.normal': 'الخط البرتقالي هو مسار الشمس أثناء الكسوف. ⛔ لا تنظر إلى الشمس دون مرشِّح.',
      'ar.calibHint': '🎯 صوّب الشبكة نحو <b>الشمس الحقيقية</b> والمس الشاشة. إن لم ترها، استعن بمَعلم معروف.',
      'ar.calibDone': '✅ صُحِّحت البوصلة بمقدار <b>{off}°</b>. سيُحفظ للمرة القادمة.',
      'ar.calibCancel': 'أُلغيت المعايرة.',
      'ar.calibReset': 'أُعيد ضبط تصحيح البوصلة.',
      'ar.noCamera': '⚠️ لا يمكن الوصول إلى الكاميرا.<br>ستظهر الطبقة التوضيحية فقط.',
      'ar.noSensors': 'لا يوجد إذن لمستشعرات الاتجاه. على iPhone أعد تحميل الصفحة واقبل تنبيه «الحركة والاتجاه».',
      'ar.sun': '☀ الشمس',
      'ar.sunBelow': '☀ الشمس (تحت الأفق)',
      'ar.turnHere': 'استدر إلى هنا',
      'ar.covered': 'محجوب',
      'ar.in': 'بعد',
      'ar.finished': 'انتهى الكسوف',
      'ar.relTag': 'بوصلة نسبية',
      'footer': 'حسابات مشتقة من <b>عناصر بيسل من NASA/GSFC</b> (F. Espenak)، وجداول VSOP87/ELP2000-85، ΔT = 71.4 ث. كل شيء يُحسب على جهازك دون خادم.<br>تم التحقق منها مقابل بيانات <b>IGN</b> الإسبانية (أوفييدو: القدر 1.015، الكلية 1د 48ث).<br><br>صُنع بمحبة لمن انتظروا هذا اليوم عقوداً. ☀️🌑',
      'v.btnOff': '🔈 تشغيل الصوت',
      'v.btnOn': '🔊 الصوت يعمل',
      'v.btnBrief': '🗣️ ماذا سأرى؟',
      'v.ready': 'تم تشغيل الصوت. سأنبهك في كل مرحلة من الكسوف.',
      'v.noVoice': 'جهازك لا يحتوي على أي صوت مثبت لهذه اللغة.',
      'v.fallback': 'هاتفك لا يحتوي على صوت عربي. سأستخدم صوتاً بلغة أخرى.',
      'v.briefWhere': 'أنت في {place}.',
      'v.briefTotal': 'من هنا سترى كسوفاً كلياً. تبدأ الكلية عند الساعة {c2} وتدوم {dur}.',
      'v.briefPartial': 'من هنا سيكون الكسوف جزئياً: سيحجب القمر {pct} بالمئة من الشمس على الأكثر.',
      'v.briefTimes': 'يبدأ عند الساعة {c1} وينتهي عند الساعة {c4}.',
      'v.briefTimesSet': 'يبدأ عند الساعة {c1}. الكسوف نفسه لا ينتهي إلا عند الساعة {c4}، لكن الشمس تغرب من هنا قبل ذلك، عند الساعة {set}: أما الجزء الأخير فيحدث والشمس تحت الأفق.',
      'v.briefSun': 'عليك أن تنظر باتجاه {dir}، ومنخفضاً جداً: لن ترتفع الشمس سوى {alt} درجات فوق الأفق، {hand}.',
      'v.briefFinger1': 'أي بعرض إصبع واحد والذراع ممدودة',
      'v.briefFingers': 'أي بعرض {n} أصابع مضمومة والذراع ممدودة',
      'v.briefFist': 'أي بعرض قبضة اليد والذراع ممدودة',
      'v.briefBlocked': 'انتبه لأمر مهم: من هذه النقطة بالذات، عند الساعة {time} ستختفي الشمس خلف التضاريس ولن تراها بعد ذلك. إن أردت الكسوف كاملاً، ابحث عن مكان أكثر انكشافاً في تلك الجهة.',
      'v.briefClear': 'خبر سار: من هنا لا يوجد أي جبل يحجب عنك الشمس.',
      'v.briefBlockedLate': 'أمر أخير: سترى الكلية، لكن عند الساعة {time} ستختفي الشمس خلف التضاريس وستفوتك المرحلة الأخيرة من الكسوف.',
      'v.briefCloudsBad': 'التوقعات حالياً ليست مشجعة: تشير إلى {low} بالمئة من السحب المنخفضة في ذلك الوقت، وهي التي تحجب الرؤية.',
      'v.briefCloudsSome': 'تشير التوقعات إلى بعض السحب المنخفضة، {low} بالمئة، في ذلك الوقت.',
      'v.briefCloudsGood': 'تشير التوقعات إلى سماء صافية في ذلك الوقت.',
      'v.briefLow': 'منخفضة جداً. تحتاج إلى أفق غربي خالٍ تماماً.',
      'v.briefFilter': 'تذكَّر: المرشِّح الشمسي موضوع دائماً، إلا أثناء الكلية.',
      'v.briefFilterAlways': 'تذكَّر: لا توجد كلية هنا. لا يجوز نزع المرشِّح الشمسي في أي لحظة.',
      'cd.tap': '🔊 المس الساعة وسأقولها لك بصوت مسموع',
      'v.greet': 'يتبقى {t} على بداية الكسوف الكلي للشمس.',
      'v.greetC2': 'بدأ الكسوف. يتبقى {t} على الكلية.',
      'v.greetNow': 'أنت في قلب الكلية. انظر إلى الأعلى!',
      'v.greetPartial': 'بدأ الكسوف. ضع المرشِّح.',
      'v.greetOver': 'كسوف 12 أغسطس قد مضى.',
      'v.greetNoEcl': 'الكسوف غير مرئي من هنا.',
      'tl.voice': 'اجعل التطبيق يتكلم',
      'tl.voiceOn': 'التطبيق يتكلم',
      'tl.voiceDesc': 'ينبهك في كل مرحلة دون أن تنظر إلى الهاتف',
      'tl.brief': 'ماذا سأرى؟',
      'tl.briefDesc': 'ملخص مسموع',
      'tl.ics': 'التقويم',
      'tl.icsDesc': 'مع تذكيرات',
      'tl.notify': 'تنبيهات',
      'tl.notifyDesc': 'إشعارات',
      'tl.share': 'مشاركة',
      'tl.shareDesc': 'أرسله',
      'tl.install': 'تثبيت',
      'tl.installDesc': 'التطبيق على الهاتف',
      'v.h1': 'تبقت ساعة على بداية الكسوف. وقت مناسب لتأخذ مكانك وتتأكد من أن الأفق الغربي خالٍ.',
      'v.m10': 'عشر دقائق على البداية. ضع نظارات الكسوف الآن.',
      'v.c1': 'بدأ الكسوف. القمر يلامس حافة الشمس. لا تنظر دون مرشِّح.',
      'v.p25': 'رُبع الشمس محجوب. انظر إلى ظل الأشجار: كل فتحة بين الأوراق ترسم على الأرض صورة هلالية للشمس.',
      'v.p50': 'نصف الشمس محجوب. يبدأ الضوء بالتغير: تصبح الألوان أكثر بهتاناً والظلال أكثر حدة.',
      'v.p75': 'ثلاثة أرباع محجوبة. ستلاحظ انخفاض الحرارة وصمت الطيور.',
      'v.m5': 'خمس دقائق على الكلية. انظر غرباً: سترى ظل القمر قادماً كجدار داكن.',
      'v.m1': 'دقيقة على الكلية. استعد لنزع المرشِّح حين أنبهك.',
      'v.s20': 'عشرون ثانية. انظر إلى سطح فاتح على الأرض: قد ترى أشرطة ظل مرتعشة.',
      'v.c2': 'الكلية! انزع النظارات. يمكنك الآن النظر إلى الشمس مباشرة.',
      'v.mid': 'أنت ترى الإكليل الشمسي. ابحث عن الزهرة، ساطعة، قرب الشمس. وانظر إلى الأفق: حولك غروب بثلاثمئة وستين درجة.',
      'v.c3warn': 'انتبه. ضع النظارات الآن.',
      'v.c3': 'انتهت الكلية. المرشِّح موضوع حتى النهاية.',
      'v.c4': 'انتهى الكسوف. إلى 2 أغسطس 2027.',
      'v.pMax': 'ذروة الكسوف. {pct} بالمئة من الشمس محجوب. لا تنزع المرشِّح في أي لحظة.',
      'mark.totality': 'كلي',
      'mark.c1': 'البداية',
      'mark.c2': 'الكلية',
      'mark.max': 'الذروة',
      'mark.c3': 'النهاية',
      'mark.c4': 'الختام',
      'v.dirSpoken': ['الشمال','شمال شمال شرقي','الشمال الشرقي','شرق شمال شرقي','الشرق','شرق جنوب شرقي','الجنوب الشرقي','جنوب جنوب شرقي','الجنوب','جنوب جنوب غربي','الجنوب الغربي','غرب جنوب غربي','الغرب','غرب شمال غربي','الشمال الغربي','شمال شمال غربي'],
      'dir': ['ش','ش ش ق','ش ق','ق ش ق','ق','ق ج ق','ج ق','ج ج ق','ج','ج ج غ','ج غ','غ ج غ','غ','غ ش غ','ش غ','ش ش غ'],
    }
  };

  const LANGS = ['ca', 'es', 'en', 'fr', 'de', 'zh', 'ar'];
  let current = 'ca';

  try {
    const saved = localStorage.getItem('eclipse-lang');
    if (saved && LANGS.indexOf(saved) >= 0) current = saved;
  } catch (e) {}

  /** Traduce una clave, sustituyendo {marcadores} */
  function t(key, params) {
    const table = DICT[current] || DICT.ca;
    let s = table[key];
    if (s === undefined) s = (DICT.ca[key] !== undefined ? DICT.ca[key] : key);
    if (params && typeof s === 'string') {
      for (const k in params) s = s.split('{' + k + '}').join(params[k]);
    }
    return s;
  }

  /** Punto cardinal para un azimut, en el idioma activo */
  function cardinal(az) {
    const names = t('dir');
    return names[Math.round(az / 22.5) % 16];
  }

  /** Aplica las cadenas estáticas a todos los [data-i18n] */
  function applyStatic() {
    document.documentElement.lang = t('html.lang');
    // El árabe se lee de derecha a izquierda. El atributo `dir` voltea por sí
    // solo el texto y las filas; el CSS solo tiene que rescatar lo que NO debe
    // voltearse (mapa, gráficas, horas).
    document.documentElement.dir = t('html.dir') || 'ltr';
    document.title = t('doc.title');
    document.querySelectorAll('[data-i18n]').forEach(el => {
      el.innerHTML = t(el.getAttribute('data-i18n'));
    });
    document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
      el.setAttribute('placeholder', t(el.getAttribute('data-i18n-placeholder')));
    });
    document.querySelectorAll('[data-i18n-title]').forEach(el => {
      el.setAttribute('title', t(el.getAttribute('data-i18n-title')));
    });
    document.querySelectorAll('.lang-btn').forEach(b => {
      b.classList.toggle('active', b.dataset.lang === current);
      b.setAttribute('aria-pressed', String(b.dataset.lang === current));
    });
  }

  const listeners = [];
  function onChange(fn) { listeners.push(fn); }

  function setLang(lang) {
    if (LANGS.indexOf(lang) < 0 || lang === current) return;
    current = lang;
    try { localStorage.setItem('eclipse-lang', lang); } catch (e) {}
    applyStatic();
    listeners.forEach(fn => { try { fn(lang); } catch (e) {} });
  }

  /**
   * Añade cadenas a los diccionarios ya cargados.
   * Las funciones nuevas traen sus traducciones en su propio fichero en vez
   * de engordar los cinco bloques de aquí.
   * @param {Object<string, Object<string,string>>} more { ca: {...}, es: {...} }
   */
  function extend(more) {
    for (const lang in more) {
      if (!DICT[lang]) DICT[lang] = {};
      const src = more[lang];
      for (const k in src) DICT[lang][k] = src[k];
    }
  }

  global.I18N = {
    t, cardinal, setLang, applyStatic, onChange, extend,
    get lang() { return current; },
    get locale() { return t('locale'); },
    LANGS
  };
  global.t = t;
})(window);
